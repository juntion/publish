...
class MayFilter extends FiltersExact implements Filter
{
    ...
    public function getSqlByTypeAndVal($query, $type, $val, $name)
    {
        switch ($type){
            ...
            case QueryType::IN_LIST_TYPE:
                if (is_array($val)){
                    return $query->orWhereIn($name, $val);
                } else {
                    return $query->orWhere($name, $val);
                }
            case QueryType::NOT_EQUAL_TYPE:
            case QueryType::NOT_IN_LIST_TYPE:
                if (is_array($val)){
                    return $query->orWhereNotIn($name, $val);
                } else {
                    return $query->orWhere($name, "!=", $val);
                }
    ...
    protected function returnSQLByTypeStr($query, $name, $val, $str)
    {
        if (is_array($val)){
            collect($val)->each(function ($item)use ($query, $name, $str){
                $query->orWhere($name, $str, $item);
            });
        } else {
            return $query->orWhere($name, $str , $val);
        }
    }
