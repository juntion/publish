...
class GatewayWorkerService implements GatewayWorkerInterface
{
    ...
    {
        $gatewayTransport = $this->config('gateway_transport', 'tcp');
        $gatewayContext = $gatewayTransport == 'ssl' ? $this->config('gateway_context', []) : [];
        $gateway = new Gateway($this->config('gateway'), $gatewayContext);  #连接服务的端口
        $gateway->transport = $gatewayTransport;
