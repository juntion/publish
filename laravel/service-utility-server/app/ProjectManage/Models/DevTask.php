...
use App\Enums\ProjectManage\DevTaskColumn;
...
class DevTask extends Model implements HasMedia
{
    ...
    use HasSourceProjectTrait, TaskStatusTrait, TaskCommonFilter, DateFormatTrait;

    protected $table = 'pm_dev_tasks';

    public const mediaCollectionName = 'devTask';
    public const TASK_PREFIX = '20';
    public const TASK_TYPE = TaskType::TASK_TYPE_DEV;
    ...
    protected $finishTypeNum = DevTaskStatus::STATUS_COMPLETED;
    protected $submitTypeNum = DevTaskStatus::STATUS_SUBMIT;
    ...
    {
        return $this->hasMany(DevSubTask::class, 'task_id');
    }
    ...
    {
        return $this->belongsToMany(Product::class, 'pm_dev_tasks_has_products');
    }
    ...
    {
        return $this->belongsToMany(ReleaseVersion::class, 'pm_dev_tasks_has_versions');
    }
    ...
    {
        return DevTaskStatus::getStatusDesc($status);
    }
    ...
    public function attributeValuesToBeLogged(string $processingEvent): array
    {
        ...
        collect($this->getDirty())->except(self::$logAttributesToIgnore)->map(function ($item, $key) use (&$changes) {
            $changes[] = [
                'name' => DevTaskColumn::COLUMN_DESC[$key],
    ...
    public function scopeStatus(Builder $builder, ...$data)
    {
        ...
        $val = $data[1];
        $searchType = $data[2];
        $const = new \ReflectionClass(DevTaskStatus::class);
        $const_arr = $const->getConstants();
        $const_name = array_search($val, $const_arr);
        if ($const_name == "STATUS_TO_ASSIGN") {
            $part_status = 0;
        } else {
            $part_status = constant(DevSubTaskStatus::class . "::" . $const_name);
        }
        if ($searchType == 'may') {
