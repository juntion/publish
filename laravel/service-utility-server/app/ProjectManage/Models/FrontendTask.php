...
use App\Enums\ProjectManage\Task\FrontendTaskColumn;
use App\Enums\ProjectManage\Task\FrontendTaskStatus;
use App\Enums\ProjectManage\TaskType;
use App\Models\BaseModel;
use App\Models\User;
use App\Observers\Task\FrontendTaskObserver;
...
class FrontendTask extends BaseModel implements HasMedia
{
    ...
    protected $table = 'pm_frontend_tasks';

    public const mediaCollectionName = 'frontendTask';
    public const TASK_PREFIX = '40';
    ...
    protected $finishTypeNum = FrontendTaskStatus::STATUS_COMPLETED;
    protected $submitTypeNum = FrontendTaskStatus::STATUS_SUBMIT;
    ...
    protected static function booted()
    {
        ...
        self::observe(FrontendTaskObserver::class);
    }

    public function subTasks()
    {
        return $this->hasMany(FrontendSubTask::class, 'task_id');
    }
    ...
    {
        return $this->belongsToMany(Product::class, 'pm_frontend_tasks_has_products');
    }
    ...
    {
        return $this->belongsToMany(ReleaseVersion::class, 'pm_frontend_tasks_has_versions');
    }
    ...
    {
        return FrontendTaskStatus::getStatusDesc($status);
    }
    ...
    public function attributeValuesToBeLogged(string $processingEvent): array
    {
        ...
        collect($this->getDirty())->except(self::$logAttributesToIgnore)->map(function ($item, $key) use (&$changes) {
            $changes[] = [
                'name' => FrontendTaskColumn::COLUMN_DESC[$key],
    ...
    public function scopeStatus(Builder $builder, ...$data)
    {
        ...
        $val = $data[1];
        $searchType = $data[2];
        $const = new \ReflectionClass(FrontendTaskStatus::class);
        $const_arr = $const->getConstants();
        $const_name = array_search($val, $const_arr);
        if ($const_name == "STATUS_TO_ASSIGN") {
            $part_status = 0;
        } else {
            $part_status = constant(FrontendSubTaskStatus::class . "::" . $const_name);
        }
        if ($searchType == 'may') {
