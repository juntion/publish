...
class FrontendTaskRepository
{
    ...
    public function store($data)
    {
        ...
        $data['status'] = $hasSubtasks ? FrontendTaskStatus::STATUS_NO_BEGIN : FrontendTaskStatus::STATUS_TO_ASSIGN;
        $frontendTask = FrontendTask::query()->create($data);
        ...
        if ($hasSubtasks) {
            ...
            $subTaskData = [
                ...
                'is_main' => 1,
                'status' => FrontendSubTaskStatus::STATUS_NO_BEGIN,
            ...
            $this->addSubTask($frontendTask, array_merge($subTaskData, $appraisalData));
        } else {
            $this->createEmptyMainSubtask($frontendTask);
        }
        ...
        if (isset($data['product_id'])) {
            ...
            $this->attachProducts($frontendTask, $product, request()->input('product_modules', []));
        }

        // 处理附件
        if (isset($data['media'])) {
            $frontendTask->addMedias($data['media']);
        }

        // 纳入版本
        if (isset($data['release_version_ids'])) {
            $frontendTask->versions()->attach($data['release_version_ids']);
        }
    }
    ...
    protected function createEmptyMainSubtask($task)
    {
        $task->subTasks()->create([
            'is_main' => 1,
            'status' => FrontendSubTaskStatus::STATUS_CLOSED,
        ]);
    }
     ...
     * @throws InvalidParameterException
     */
    public function updateTask(FrontendTask $task, Request $request)
    {
        ...
        if (request()->has('main_sub_task')) {
            $mainSubTaskData = $data['main_sub_task'];
            $mainSubTask = FrontendSubTask::query()->where('id', $mainSubTaskData['sub_task_id'])->first();
            $appraisalData = $this->subTaskAppraisalData($mainSubTaskData['standard_workload']);
        ...
        if (request()->has('other_sub_tasks')) {
            $otherSubTasks = $data['other_sub_tasks'];
            foreach ($otherSubTasks as $item) {
                $otherSubTask = FrontendSubTask::query()->where('id', $item['sub_task_id'])->first();
                $appraisalData = $this->subTaskAppraisalData($item['standard_workload']);
     ...
     * @return bool
     */
    public function update(FrontendTask $task, array $data)
    {
    ...
    {
        $teams = Team::query()->where('type', TeamType::TYPE_FRONTEND);
        // 有搜索条件
        ...
        if ($demand = $task->demand()->first()) {
            return $this->teams->getTeamPrincipalByProducts($demand, TeamType::TYPE_FRONTEND);
        }
        ...
        // 无产品
        return $this->dropDownTask->frontendPrincipal();
    }
     ...
     * @throws InvalidParameterException
     */
    public function expirationDate(FrontendTask $task, array $data)
    {
     ...
     * @param array $data
     */
    public function verify(FrontendTask $task, array $data)
    {
        $principal = User::query()->find($data['user_id']);
        $data['principal_user_id'] = $principal->id;
        $data['principal_user_name'] = $principal->name;
        $data['status'] = FrontendTaskStatus::STATUS_CLOSED;
        if ($demand = $task->demand()->first()) {
            $designTask = $demand->designTasks()->whereNotIn('status', [FrontendTaskStatus::STATUS_COMPLETED, FrontendTaskStatus::STATUS_REVOCATION])->get();
            if ($designTask->isEmpty()) {
                $data['status'] = FrontendTaskStatus::STATUS_TO_ASSIGN;
            }
            // 更新demand_links的开发环节负责人
            $demandLik = $demand->demandLinks()->where('type', DemandLinksType::TYPE_FRONTEND)->first();
            $demandLik->update([
     ...
     * @param array $data
     */
    public function verifyUpdate(FrontendTask $task, array $data)
    {
        $principal = User::query()->find($data['user_id']);
        $data['principal_user_id'] = $principal->id;
        $data['principal_user_name'] = $principal->name;
     ...
     * @return FrontendTask
     */
    public function details(FrontendTask $task)
    {
     ...
     * @param $data
     */
    public function createSubTask(FrontendTask $task, $data)
    {
        DB::transaction(function () use ($task, $data) {
            foreach ($data as $item) {
                $item['handler_id'] = $item['user_id'];
                $item['handler_name'] = User::find($item['user_id'])->name;
                $item['status'] = FrontendSubTaskStatus::STATUS_NO_BEGIN;
                $appraisalData = $this->subTaskAppraisalData($item['standard_workload']);
                $this->addSubTask($task, array_merge($item, $appraisalData));
            }
            if ($task->status == FrontendTaskStatus::STATUS_SUBMIT) {
                $task->update(['status' => FrontendTaskStatus::STATUS_IN_PROGRESS]);
            }
        });
    }
     ...
     * @throws InvalidParameterException
     */
    public function setHandler(FrontendTask $task, array $data)
    {
        ...
        $data['status'] = FrontendSubTaskStatus::STATUS_NO_BEGIN;
        $task->update(['expiration_date' => $data['expiration_date']]);
        $mainSubTask = $task->subTasks()->where('is_main', 1)->first();
        // 考核数据
        $appraisalData = $this->subTaskAppraisalData($data['standard_workload']);
     ...
     */
    public function subTaskStart(FrontendSubTask $subTask)
    {
        $subTask->update([
            'status' => FrontendSubTaskStatus::STATUS_IN_PROGRESS,
     ...
     * @throws InvalidParameterException
     */
    public function subTaskSubmit(FrontendSubTask $subTask, array $data)
    {
        $data['share_address'] = isset($data['share_address']) ? json_encode($data['share_address']) : null;
        $data['status'] = FrontendSubTaskStatus::STATUS_SUBMIT;
     ...
     * @param array $data
     */
    public function subTaskSubmitUpdate(FrontendSubTask $subTask, array $data)
    {
        $data['share_address'] = isset($data['share_address']) ? json_encode($data['share_address']) : null;
        $subTask->update($data);
        ...
        if (isset($data['comment'])) {
            $lastSubmitLog = $subTask->statusLogs()->where('new_status', FrontendSubTaskStatus::STATUS_SUBMIT)->orderBy('id', 'desc')->first();
     ...
     * @param FrontendSubTask $subTask
     */
    public function subTaskSubmitCancel(FrontendSubTask $subTask)
    {
        $subTask->update([
            'status' => FrontendSubTaskStatus::STATUS_IN_PROGRESS,
            'release_type' => null,
     ...
     * @param FrontendSubTask $subTask
     */
    public function subTaskRevocation(FrontendSubTask $subTask)
    {
        $this->subTaskStatus($subTask, FrontendSubTaskStatus::STATUS_REVOCATION);
        $subTask->clearMedias();
    }
     ...
     * @return bool
     */
    public function subTaskStatus(FrontendSubTask $subTask, $status)
    {
     ...
     * @param FrontendSubTask $subTask
     */
    public function subTaskPause(FrontendSubTask $subTask)
    {
        $subTask->update([
            'status' => FrontendSubTaskStatus::STATUS_PAUSED,
            'pause_time' => now(),
        ]);
    }
     ...
     * @throws InvalidParameterException
     */
    public function subTaskComplete(FrontendSubTask $subTask, array $data)
    {
        if ($data['result'] == 0) {
            $subTask->update(['status' => FrontendSubTaskStatus::STATUS_IN_PROGRESS]);
        } else {
            // 验收通过
            ...
            $subTaskData = [
                'status' => FrontendSubTaskStatus::STATUS_COMPLETED,
            ...
            $offsetFactor = FrontendSubTask::getOffsetFactor($offsetDays);
     ...
     * @throws InvalidParameterException
     */
    public function subTaskExpirationDate(FrontendSubTask $subTask, array $data)
    {
        $appraisalData = $this->subTaskAppraisalData($data['standard_workload']);
     ...
     * @param array $data
     */
    public function subTaskUpdateVersion(FrontendSubTask $subTask, array $data)
    {
