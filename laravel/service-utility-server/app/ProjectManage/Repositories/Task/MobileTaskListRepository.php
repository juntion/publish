...
class MobileTaskListRepository extends BaseRepository
{
    use RemindTrait;
    ...
    public function __construct(MobileTask $task)
    {
        $this->model = $task;
    }
    ...
    public function getList($limit)
    {
        ...
        $result->load(['ownProducts', 'media', 'versions.product', 'project',
            ...
            'demand.products.teams' => function ($query) {
                $query->where('type', TeamType::TYPE_MOBILE);
            },
            'demand.demandLinks' => function ($query) {
                $query->where('type', DemandLinksType::TYPE_MOBILE);
            },
        ]);
        ...
        foreach ($result as $task) {
            ...
            foreach ($task->subTasks as $subTask) {
                ...
                $lastSubmitLog = $subTask->statusLogs->sortByDesc('id')->where('new_status', MobileSubTaskStatus::STATUS_SUBMIT)->first();
