...
class TestTaskExport implements WithTitle, WithEvents, WithColumnFormatting
{
    use Exportable, PMExportTrait;

    protected $headers = [
        '发布时间', '发布部门', '发布人', 'P级别', '产品类别', ' 需求号/需求标题', '总任务ID', '负责人', '子任务ID',
    ...
    public function __construct(TestTaskListRepository $testTaskList)
    {
        $this->data = $testTaskList->exportData();
    }
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $this->setCommonPart($workSheet, 'N');

                foreach ($this->data as $testTask) {
                    $currentLine = $this->headLine() + $this->count + 1;
                    $workSheet->setCellValue('A' . $currentLine, Carbon::parse($testTask->created_at)->toDateString());
                    $workSheet->setCellValue('B' . $currentLine, $this->getBasicDept($testTask->promulgatorUser));
                    $workSheet->setCellValue('C' . $currentLine, $testTask->promulgator_name);
                    $workSheet->setCellValue('D' . $currentLine, $testTask->priority ?? '-');
                    $workSheet->setCellValue('E' . $currentLine, $this->getProducts($testTask));
                    $workSheet->getStyle('E' . $currentLine)->getAlignment()->setWrapText(true);
                    $workSheet->setCellValue('F' . $currentLine, $this->getDemandInfo($testTask));
                    $workSheet->getStyle('F' . $currentLine)->getAlignment()->setWrapText(true);
                    $workSheet->setCellValue('G' . $currentLine, $testTask->number);
                    $workSheet->setCellValue('H' . $currentLine, $testTask->principal_user_name);
                    $workSheet->setCellValue('N' . $currentLine, $testTask->status_desc);

                    $testIndex = 0;
                    foreach ($testTask->subTasks as $testSubTask) {
                        $workSheet->setCellValue('I' . ($currentLine + $testIndex), $testSubTask->number);
                        $workSheet->setCellValue('J' . ($currentLine + $testIndex), $testSubTask->handler_name ?? '-');
                        $workSheet->setCellValue('K' . ($currentLine + $testIndex), $testSubTask->expiration_date ?? '-');
                        $workSheet->setCellValue('L' . ($currentLine + $testIndex), $this->getFinishDesc($testSubTask));
                        $workSheet->setCellValue('M' . ($currentLine + $testIndex), $testSubTask->status_desc);
                        $testIndex++;
                    }

                    $taskCount = $testTask->subTasks->count();
                    ...
                    if (($mergeCell = $taskCount) > 1) {
                        ...
                        $workSheet->mergeCells('N' . $currentLine . ':' . 'N' . ($currentLine + $mergeCell - 1));
                    }
                }
            },
