...
class ERPProductsModel extends Seeder
{
    public function run()
    {
        $export = Product::query()->select('products_model')
            ->where('products_model', '!=', "")
            ->whereNotNull('products_model')
            ->groupBy('products_model')
            ->get();
        $user = Admin::query()->where('name', config('app.root'))->first();

        $this->command->info('开始迁移 erp products 表提取products_model信息');
    ...
    private function migrate(&$item, $uuid)
    {
        try {
            ...
            $this->error('导入Products数据->数据表：resource_tags，name = '.$item->products_model.',异常信息：'.$e->getMessage());
        }
    }
}
