...
class CompanyWarehouseRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            ...
            'postcode'             => 'required',
            'foreign_postcode'     => 'required',
            'contacts'             => 'array',
            'contacts.*.name'      => 'required',
            'contacts.*.tel'       => 'required',
            'contacts.*.type'      => 'required',
            'contacts.*.uuid'      => 'exists:Modules\Base\Entities\Company\CompanyAddressContact,uuid',
            'uuid'                 => [
                Rule::exists('company_addresses', 'uuid')->where('type', AddressType::WAREHOUSE_TYPE)
            ]
        ];
    }
