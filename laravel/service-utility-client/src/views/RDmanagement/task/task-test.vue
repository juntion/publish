<template>
  <div>
      <addFollower :postType="2" ref="addFollower" title="测试跟进人" :id="subtasks_id"></addFollower>
      <allTaskInfo  ref="allTaskInfo" :id="demand_id" title="其他环节信息"></allTaskInfo>
      <!--审核弹窗-->
          <div class="modal-box">
              <a-modal :title="verifyType === 1 ? '测试任务审核' : '更改测试任务审核'"
                       destroyOnClose
                       :maskClosable="false"
        ...
        <div class="radio_box"
          ...
          <a-table :dataSource="data2"
                ...
                <div slot="status" slot-scope="status,record">
                    ...
                    <span style="color:#FF4A4A;" v-if="record.status=='待发布'">{{record.status}}</span>
    ...
    <div class="tabslist">
      <a-tabs class="tabs_bg"
        ...
        <a-tab-pane :key="9">
          <span slot="tab" title="测试主负责人需要审核任务等级和实际负责人">
            待审核
            <a-badge :count="remind.status9" />
          </span>
        </a-tab-pane>
        <a-tab-pane :key="0">
          <span slot="tab" title="上一环节还未完成的任务">
            等待中
        ...
        <a-tab-pane :key="1">
         ...
         <a-tab-pane :key="8">
          <span slot="tab" title="未发布测试站的任务">
            待发布
            <a-badge :count="remind.status8" />
          </span>
        </a-tab-pane>
        <a-tab-pane :key="2">
          <span slot="tab" title="已发布测试站未开始测试的任务">
            待测试
            <a-badge :count="remind.status2" />
          </span>
        </a-tab-pane>
        <a-tab-pane :key="3">
          <span slot="tab" title="正在测试的任务">
            测试中
            <a-badge :count="remind.status3" />
          </span>
        </a-tab-pane>
           <a-tab-pane :key="7">
          <span slot="tab" title="已提交测试结果给负责人审查的任务">
            已提交
      ...
      <div class="select-box">
        <!-- <a-select placeholder="发布人"
                  ...
                  showSearch
                  optionFilterProp="children"
                  v-model="searchData.testTaskPublisher"
        ...
        <!-- <a-select placeholder="负责人"
                  ...
                  showSearch
                  optionFilterProp="children"
                  v-model="searchData.testPrincipal"
                  style="width: 7%;margin-right: 10px;">
        ...
        <!-- <a-select placeholder="处理人"
                  ...
                  showSearch
                  optionFilterProp="children"
                  v-model="searchData.testTaskHandler"
                  style="width: 7%;margin-right: 10px;">
        ...
        <a-select placeholder="优先级"
          ...
          <a-select placeholder="处理人状态"
                ...
                <!-- 0：待测试；1：测试中；2：已完成；3：已暂停；4：已撤销；5：已提交；6：待发布；7：关闭中； -->
                <a-select-option :value="0"> 待测试</a-select-option>
                ...
                <a-select-option :value="6"> 待发布</a-select-option>
                <a-select-option :value="7"> 关闭中</a-select-option>
        </a-select>
         <a-range-picker
        ...
        <span style="margin-left:10px;color: #378eef">
            <mySearch @search="moreSearch" ref="search" type="test"></mySearch>
        </span>
        <div class="upload_box">
          <div class="popup_opinion_submit_box after">
            <ul class="popup_opinion_submit_file">
              <span>筛选：</span>
              <li v-if="searchData.testTaskPublisher"><b>发布人：{{searchData.testTaskPublisher.label}}</b>
              ...
              <li v-if="searchData.testPrincipal"><b>负责人：{{searchData.testPrincipal.label}}</b>
                <i class="icon iconfont"
                   @click="reset(4)">&#xe631;</i>
              </li>
              <li v-if="searchData.testTaskHandler"><b>处理人：{{searchData.testTaskHandler.label}}</b>
                <i class="icon iconfont"
                   @click="reset(5)">&#xe631;</i>
              </li>
                ...
                <a-row class="form-row">
                  ...
                  <a-form-item label="指定截止时间" style="margin-top:15px">
                    <a-date-picker style="width:245px"
                     ...
                     <span v-if="addForm.getFieldValue('expiration_date')" style="margin-left:6px">
        ...
        <!-- 提交 -->
        <div class="modal-box">
          <el-dialog :title="type===1 ? '提交测试反馈' : '信息更新'"
                      destroy-on-close
            ...
            <div class="radio_box">
              ...
              <p style="margin-top:20px">反馈结果</p>
              <a-radio-group v-model="result">
                <a-radio :value="0">无故障</a-radio>
                <a-radio :value="1">故障已全部解决</a-radio>
                <a-radio :value="2">故障未全部解决</a-radio>
              </a-radio-group>

              <p style="margin-top:20px">反馈内容</p>
              <el-input type="textarea"
                        :autosize="{ minRows: 3, maxRows: 3}"
                        placeholder="请输入相关反馈内容"
                        v-model="comment"
                        resize="none">
              </el-input>
              <a-row style="margin-bottom:10px;margin-top:20px">
                ...
                <a-row class="form-row" style="margin-bottom:15px">
                  ...
                  <a-col :lg="12"
                    ...
                    <a-form-item label="预计交付日期">
                      ...
                      <span v-if="allotForm.getFieldValue('expiration_date')" style="margin-left:6px">
                        ...
                        <div class="top">
                           <span class="text-p-overflow dept" :title="taskMsg.title" style="max-width:422px">{{taskMsg.title}}</span>
                            ...
                            <span class="status">
                                ...
                                <span  v-if="taskMsg.status_desc=='待测试'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{taskMsg.status_desc}}</span>
                                <span  v-if="taskMsg.status_desc=='测试中'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{taskMsg.status_desc}}</span>
                                ...
                                <span  v-if="taskMsg.status_desc=='待发布'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{taskMsg.status_desc}}</span>
                            </span>
                        </div>
                        <div class="con">
                            <a-tabs type="card" v-model="activeKey">
                                <a-tab-pane key="1">
                                    ...
                                    <div style="margin-bottom:20px">
                                        ...
                                        <span class="right">
                                            <span v-if="taskMsg.demand">需求任务/测试</span>
                                ...
                                <a-tab-pane key="2">
                                    ...
                                    <div class="header">
                                        ...
                                        <span class="pdR10">测试负责人:</span><span>{{taskMsg.principal_user_name}}</span>
                                        ...
                                        <div  v-for="(task,index) in taskMsg.subtasks" :key="index" style="padding:30px 0" :style="{'border-bottom': index === taskMsg.subtasks.length-1 ? '0' : '1px solid #EBF3FD'}">
                                            ...
                                            <div class="marginB20" style="display:flex">
                                                <div style="flex:1" >
                                                    ...
                                                    <span style="color:#FEBC2E;" v-if="task.status_desc=='待测试'">{{task.status_desc}}</span>
                                                    <span style="color:#FEBC2E;" v-if="task.status_desc=='测试中'">{{task.status_desc}}</span>
                                                    ...
                                                    <span style="color:#FEBC2E;" v-if="task.status_desc=='待发布'">{{task.status_desc}}</span>
                                                </div>
                                                <div style="flex:1">
                                            ...
                                            <div>
                                                ...
                                                <span>{{task.feedback ? task.feedback : '--'}}</span>
                                            </div>
                                        </div>
        ...
        <div class="btn-right">
          ...
          <a-button type="primary"
                    v-if="canDo('pm.tasks.test.store')"
                    @click="releaseTask">
                            ...
                            <tr>
                                            ...
                                            <span v-if="item.task_versions.length===0">
                                                 <span  v-for="(k,index) in item.expected_versions" :key="index">
                                                     <a-popover placement="bottom" >
                                                       ...
                                                       <span class="version-tag-2">{{k.full_version}}</span>
                                                    </a-popover>
                                                </span>
                                            </span>
                                ...
                                <th style="width:7%">
                                    <span class="cup status" title="点击查看状态详情" @click="getLogs(1,item)">
                                        ...
                                        <span  v-if="item.status_desc=='待测试'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{item.status_desc}}</span>
                                        <span  v-if="item.status_desc=='测试中'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{item.status_desc}}</span>
                                        ...
                                        <span  v-if="item.status_desc=='待发布'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{item.status_desc}}</span>
                                    </span>
                                </th>
                                ...
                                <tr v-for="(item2,index) in item.subtasks" :key="item2.number">
                                   ...
                                   <td style="width:18%" :class="{'bbb' : item2.status_desc==='已撤销'}">
                                    ...
                                    <td style="width:7%" :class="{'bbb' : item2.status_desc==='已撤销'}">
                                        ...
                                        <span  v-if="item2.status_desc=='待测试'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{item2.status_desc}}</span>
                                        <span  v-if="item2.status_desc=='测试中'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{item2.status_desc}}</span>
                                        ...
                                        <span  v-if="item2.status_desc=='待发布'"><span style="color:#FEBC2E;" class="iconfont fz13">&#xe654;</span> {{item2.status_desc}}</span>
                                    </span>
                                    </td>
                                    <td style="width:100px">
                                            ...
                                            <span class="icon iconfont"
                                                ...
                                                @click="dialogVisible3=true,taskId=item2.id,expiration_date = item2.expiration_date">&#xe647;</span>
                                            <span class="icon iconfont"
                                                title="提交"
                                                v-if="item2.policies.submit"
                                                @click="submitModal(item2,1)">&#xe65b;</span>
                                            <span class="icon iconfont"
                                                title="撤销提交"
...
<script>
import { canDo, filtering, removeProperty } from '@/plugins/common'
import qs from 'qs'
...
import { testTaskPublisher, getTestPrincipal, testTaskHandler, getNewTestPrincipal } from '@/api/RDmanagement/dropDown'
import allTaskInfo from '@/components/allTaskInfo'
import { submitUpdate, getExcel, handlerTask, postTestTask, getTestTask, setPriority, setSubtasksPriority, statsuChangeLog, subtasksChangeLog, startTask, completeTask, submitTask, dissubmitTask, revocationTask, stopTask, changeSubtasksDate, changeTaskDate, verifyTask, verifyUpdateTask } from '@/api/RDmanagement/task/test'
...
let sort = []
export default {
  components: { addFollower, myViewer, downPrd, downMedia, operationLogs, mySearch, allTaskInfo, allPersonSelect },
  data () {
    return {
      ...
      searchData: {
        ...
        testTaskPublisher: undefined,
        testPrincipal: undefined,
        testTaskHandler: undefined,
        proId: undefined
      },
      data: [],
      columns,
  ...
  created () {
    ...
    if (userType.link_type === 'test') {
      if (userType.task_leader && !this.$route.query.number) {
        ...
        this.searchData.testPrincipal = { label: user.name, key: user.id }
        ...
        this.searchData.testTaskHandler = { label: user.name, key: user.id }
        ...
        this.searchData.testTaskPublisher = { label: user.name, key: user.id }
      }
    }
    testTaskPublisher().then(res => {
      if (res.code === 200) {
  ...
  watch: {
    ...
    searchData: {
      handler (newVal, oldVal) {
        ...
        if (newVal.testTaskPublisher) {
          search['promulgator_id'] = newVal.testTaskPublisher.key
        } else {
          search['promulgator_id'] = undefined
        }
        if (newVal.testPrincipal) {
          search['principal_user_id'] = newVal.testPrincipal.key
        } else {
          search['principal_user_id'] = undefined
        }
        if (newVal.testTaskHandler) {
          search['subTasks.handler_id'] = newVal.testTaskHandler.key
        } else {
        ...
        this.loading = true
        let params = { append: ['operation_log'],
          filters: search,
          may,
          must,
  ...
  methods: {
    ...
    removeProperty,
    handleSearch1 (e) {
      this.searchData.testTaskPublisher = e.id === undefined ? undefined : { key: e.id, label: e.name }
    },
    handleSearch4 (e) {
      this.searchData.testPrincipal = e.id === undefined ? undefined : { key: e.id, label: e.name }
    },
    handleSearch5 (e) {
      this.searchData.testTaskHandler = e.id === undefined ? undefined : { key: e.id, label: e.name }
    },
    handleModalSearch (e) {
    ...
    submitModal (record, status) {
      if (status === 2) {
        this.comment = record.feedback
    ...
    moreSearch (e) {
      ...
      for (let key in this.searchData) {
        if (key === 'testTaskPublisher' || key === 'testPrincipal' || key === 'testTaskHandler') {
    ...
    reset (index) {
      if (index === 1) {
        ...
        this.TaskPublisherID = undefined
        this.$refs.TaskPublisherRef.value = undefined
        this.searchData.testTaskPublisher = undefined
      } else if (index === 2) {
        ...
        this.PrincipalID = undefined
        this.$refs.PrincipalRef.value = undefined
        this.searchData.testPrincipal = undefined
      } else if (index === 5) {
        this.TaskHandlerArr = []
        this.TaskHandlerID = undefined
        this.$refs.TaskHandlerRef.value = undefined
        this.searchData.testTaskHandler = undefined
      } else if (index === 6) {
        this.mySearch = false
        this.loading = true
        may = []
    ...
    onClose () {
      ...
      this.activeKey = '1'
      if (this.$refs.test) {
        this.$refs.test.form.resetFields()
      }
    },
    serchFocus (val) {
      if (this.verifyType === 1) {
        ...
        getNewTestPrincipal(this.taskId, params).then(res => {
          if (res.code === 200) {
    ...
    showPeopleSelect (record, type) {
      ...
      if (type === 1) {
        getNewTestPrincipal(record.id).then(res => {
          if (res.code === 200) {
            this.options3_1 = res.data.users
          }
        })
        ...
        getTestPrincipal(record.id).then(res => {
          if (res.code === 200) {
            this.options3_1 = res.data.users
          }
        })
    ...
    showHandler (record, item2) {
      if (record.policies.setHandler && item2.is_main) {
        ...
        if (record.main_subtask.length) {
          if (!record.main_subtask[0].handler_id) {
          ...
          setTimeout(() => {
            this.allotForm.setFieldsValue({
              ...
              'comment': record.main_subtask[0].description,
              'expiration_date': day
            })
          }, 0)
    ...
    //   this.dialogVisible1 = true
      this.$router.push({ name: this.$route.query.project ? 'projectReleaseTask' : 'releaseTask', query: { link: 'test', project: this.$route.query.project ? 1 : undefined } })
    },
    goEdit (e) {
      this.$router.push({ name: this.$route.query.project ? 'projectReleaseTask' : 'releaseTask', query: { link: 'test', id: e.id, project: this.$route.query.project ? 1 : undefined } })
    },
    cancel (index) {
      if (index === 1) {
    ...
    ok (index) {
      ...
      if (index === 1) {
        ...
        if (this.comment) {
          formData.append('feedback', this.comment)
        }
