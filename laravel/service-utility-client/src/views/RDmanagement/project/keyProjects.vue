    ...
    <a-form :form="form">
      <div class="box">
        <div class="header">
          <h1>创建重点项目</h1>
          <p v-if="delTipshow">
           可创建一个项目，为其指定相关负责人，请仔细填写相关信息
             <!-- <span class="icon iconfont down" @click="delTip">&#xe631;</span> -->
          </p>
        </div>
        <div class="con">
          <h3>基本信息</h3>
          <div class="info">
            <a-row class="form-row">
              <a-col :lg="12"
                ...
                <a-form-item :validate-status="validateStatus.name"
                             :help="validateStatus.nameTxt">
                  <span slot="label">项目名称</span>
                  <a-input placeholder="请输入项目名称"
                             :maxLength="40"
                           v-decorator="['name',{ rules: [{ required: true, message: '请输入项目名称' },{ max:40,message: '* 请将项目名称控制在40个字符以内', }] }, ]" />
                </a-form-item>
              </a-col>
              <a-col :lg="12"
                     :md="12"
              ...
              <a-col :lg="12"
                ...
                <a-form-item>
                  ...
                  <a-select v-decorator="['level',{ rules: [{ required: true, message: '请选择项目级别' }] }, ]"
                            ...
                            style="width: 100%">
                    <a-select-option value="S"><span style="color:rgba(255, 74, 74, 1)"> S</span></a-select-option>
                    <a-select-option value="A"><span style="color:rgba(248, 141, 73, 1)"> A</span></a-select-option>
                    <!-- <a-select-option value="B"><span style="color:rgba(248, 141, 73, 1)"> B</span></a-select-option>
                    <a-select-option value="C"><span style="color:rgba(254, 188, 46, 1)"> C</span></a-select-option>
                    <a-select-option value="D"><span style="color:rgba(254, 188, 46, 1)"> D</span></a-select-option> -->
                  </a-select>
                </a-form-item>
              </a-col>
              <a-col :lg="12"
            ...
            <a-row class="form-row">
              <a-col :lg="12"
                ...
                <a-form-item :validate-status="validateStatus.principal_user_id"
                  ...
                  <peopleSelect @getValue2="getChargeValue"></peopleSelect>
                </a-form-item>
              </a-col>
              <a-col :lg="12"
                     :md="12"
...
import { pushKeyprojects } from '@/api/RDmanagement/project'
...
export default {
  components: { myEditor, peopleSelect, multiplePeopleSelect },
  data () {
    return {
      delTipshow: true,
      btnLoad: false,
      ...
      form1: {
        ...
        expiration_date: '',
        contents: '<p><strong>项目起因:</strong></p><p><br></p><p><br></p><p><strong>项目意义:</strong></p><p><br></p><p><br></p>',
        status: 1,
        shared_address: [
          { value: '' }
        ],
  ...
  methods: {
    ...
    getChargeValue (e) {
      if (JSON.stringify(e).indexOf('user') === -1) {
        this.form1.principal_user_id = e
      } else {
        this.form1.principal_user_id = e.replace('user-', '')
      }
    },
    getCareValue (e) {
    ...
    postForm () {
      this.form.validateFields((err, values) => {
        ...
        if (!err && this.form1.principal_user_id && this.form1.contents) {
          ...
          this.btnLoad = true
          // 创建重点项目
          pushKeyprojects(formData).then(res => {
            if (res.code === 200) {
              this.$message.success('创建重点任务成功')
