      ...
      <div class="select-box">
        ...
        <a-select placeholder="优先级"
         ...
         <a-range-picker
          ...
          <div  style="width: 14%;position:relative;top: -0.5px;display: inline-block">
      ...
      <div class="table-list">
        <a-table :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}"
          ...
          <div slot="buttons"
               ...
               class="button_box" @click="handbutton(record)">
          ...
          <div slot="source_project_name"
               ...
               class="pro_status">
            <p class="text-p text-p-blue"><router-link :to="{ name: 'proDetails', query: { id: record.source_project_id }}" target="_blank" class="text-p-overflow" :title="source_project_name" style="max-width:200px">  {{source_project_name}}</router-link></p>
            <p class="text-p"
...
import { getProductPrincipal, getDemandPublisher, getDemandsPrincipal, getAnalysisPeople, eidtDemandsPrincipal, exportKpiExcel, getExcel } from '@/api/RDmanagement/dropDown'
import { getProducts } from '@/api/RDmanagement/ProductMaintenance/index.js'
import { getDemands, pushPriority, continueDemands, stopDemands, testDemands, revokeDemands, completeDemands, demandChangeLog, verifyDemands, demandsTransfer } from '../../../api/RDmanagement/product/index'
const columns = [
  {
    title: 'P级别',
    dataIndex: 'priority',
...
export default {
  ...
  created () {
    if (localStorage.getItem('isReload2')) {
      this.$store.commit('changeGuide2', false)
    } else {
      this.$store.commit('changeGuide2', true)
  ...
  methods: {
    ...
    moreSearch (e) {
      ...
      for (let key in this.searchData) {
        if (key === 'productPrincipal' || key === 'demandPublisher') {
    ...
    releaseDemand () {
      this.$router.push({ name: 'releaseDemand' })
    },
...
.modal-task /deep/ .ant-table-tbody tr:nth-of-type(even){
    background: #f8f8f8;
}
