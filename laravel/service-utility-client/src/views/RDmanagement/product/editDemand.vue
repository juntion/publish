    ...
    <a-form :form="form">
      <div class="box">
              ...
              <a-col :lg="12"
                ...
                <a-form-item label="需求标题">
                  <a-input placeholder="请输入需求标题,简洁清晰,突出要点"
                            :maxLength="40"
                           v-decorator="['name', { rules: [{ required: true, message: '请输入需求标题' }] }]" />
                </a-form-item>
              </a-col>
              <a-col :lg="12"
                     :md="12"
              ...
              <a-col :lg="12"
                ...
                <a-form-item>
                  ...
                  <multiplePeopleSelect @getValue2="getCareValue"
                                        ref="attentionUser"></multiplePeopleSelect>
                </a-form-item>
              </a-col>
            </a-row>
        ...
        <div class="con">
          ...
          <a-form-item>
            <div class="info">
              <p class="mb10"><span style="color:#FF0000">*</span> 研发环节:</p>
              <div class="mb20 fz12">
              ...
              <div v-if="checkedList.indexOf('测试') !== -1">
                <p class="mb10"><span style="color:#FF0000">*</span> 测试人员:</p>
                <div class="mb20 fz12">
                  <a-radio-group v-model="form1.demand_links.test.group" :disabled="typeDisabled">
          ...
          <a-form-item :validate-status="validateStatus.media"
            ...
            <div v-for="(item, index) in fileInputList"
              ...
              <div @click="() => removeFileInputList(index)"
                   class="delFile"> <span class="iconfont" style="top: 4px;position: relative;">&#xe64d;</span></div>
        ...
        <div class="con">
          <a-button @click="postForm"
                    ...
                    type="primary">{{status=== '审核驳回'? '重新立项' : '保存'}}</a-button>
...
import { getDemandsDetails, editDemands } from '../../../api/RDmanagement/product/index.js'
...
export default {
  ...
  data () {
    return {
      ...
      btnLoad: false,
      active: false,
      form: this.$form.createForm(this, { name: 'editDemand' }),
      validateStatus: {
        media: 'success',
        mediaTxt: '',
        contents: 'success',
  ...
  created () {
    ...
    getDemandsDetails(this.$route.query.id).then(res => {
      this.sqProduct = res.data.demand.appeals.map(item => {
        return item.product_category
      })
      this.sqNumber = res.data.demand.appeals.map(item => {
        return item.number
      })
      this.status = res.data.demand.status_desc
      if (res.data.demand.status === 0 | res.data.demand.status === 1 | res.data.demand.status === 2) {
        this.statusDisabled = false
      } else {
        this.statusDisabled = true
      }
      if (res.data.demand.status === 0 | res.data.demand.status === 1 | res.data.demand.status === 2 | res.data.demand.status === 3) {
        this.proDisabled = false
      } else {
        this.proDisabled = true
      }
      if (res.data.demand.status === 6 | res.data.demand.status === 7 | res.data.demand.status === 8 | res.data.demand.status === 10) {
      ...
      let attentionIds = []
      let Arr = []
      res.data.demand.user_attention.forEach(item => {
        let data = {}
      ...
      getProducts(res.data.demand.product_category.product_line.id).then(res => {
        this.products = res.data.products
      })
      getProducts(res.data.demand.product_category.product.id).then(res => {
        this.modules = res.data.products
      })

      this.productsArr = res.data.demand.products

      this.form1.content = res.data.demand.content
      var day = ''
      if (res.data.demand.expiration_date) {
        day = moment(res.data.demand.expiration_date)
      } else {
        day = undefined
      }
      this.form1.release_version_ids = res.data.demand.expected_versions.map(item => {
        return item.id
      })
      if (res.data.demand.demand_links.length > 0) {
        res.data.demand.demand_links.forEach(item => {
          if (item.type === 1) {
            this.form1.demand_links.design = item
            this.checkedList.push('设计')
          } else if (item.type === 2) {
            this.form1.demand_links.dev = item
            this.checkedList.push('程序')
          } else if (item.type === 3) {
            this.checkedList.push('测试')
            this.form1.demand_links.test = item
          }
        })
      } else {
        this.checkedList = []
      }
      if (!res.data.demand.priority) {
        res.data.demand.priority = undefined
      }
      setTimeout(() => {
        this.form.setFieldsValue({
          'name': res.data.demand.name,
          'priority': res.data.demand.priority,
          'expiration_date': day,
          'productsLine': res.data.demand.product_category.product_line.id,
          'products': res.data.demand.product_category.product.id,
          'source_project_name': res.data.demand.source_project_name,
          'checkedList': this.checkedList
        })
      }, 0)
      let a = []
      res.data.demand.product_category.product_modules.forEach(item => {
      ...
      if (res.data.demand.product_category.product_modules.length === 0) {
        this.form1.product_modules = [{ module_id: undefined, label_ids: [], moduleTags: [] }]
      } else {
        this.form1.product_modules = a
      }

      this.form1.source_project_id = res.data.demand.source_project_id
      this.form1.source_project_name = res.data.demand.source_project_name
      if (!res.data.demand.share_address) {
        this.value = 0
      } else {
        let url = []
        let obj = {}
        res.data.demand.share_address.forEach(item => {
          obj = { value: item }
          url.push(obj)
        })
        this.form1.share_address = url
      }
      this.fileInputList = res.data.demand.media
    })
  },
  mounted () {
    // 获取项目列表
    ...
    postForm () {
      this.form.validateFields((err, values) => {
        ...
        if (!err && this.form1.content && this.validateStatus.media === 'success') {
          ...
          if (this.checkedList.indexOf('设计') !== -1) {
          ...
          if (this.checkedList.indexOf('测试') !== -1) {
          ...
          if (values.source_project_name) {
            if (values.source_project_name === this.form1.source_project_name) {
              formData.append('source_project_id', this.form1.source_project_id)
              formData.append('source_project_name', this.form1.source_project_name)
            } else {
          ...
          this.btnLoad = true
          // 编辑需求接口
          editDemands(this.$route.query.id, formData).then(res => {
            if (res.code === 200) {
              this.$message.success('编辑需求成功')
