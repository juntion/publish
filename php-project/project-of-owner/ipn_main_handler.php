...
Processing...
  ...
  /******************************/
  $debug = false;
  if ($debug) $log_file = fopen('ipn.log.txt', 'a+');
  if ($debug)fwrite($log_file, 'beginning ...'."\n");
  if ($debug)fwrite($log_file, 'session data: '. var_export($_SESSION,true) . "\n");
  if ($debug)fwrite($log_file, 'post data from paypal:'. var_export($_POST,true) . "\n");
  ...
  /**************************/
  if ($debug) fwrite($log_file, 'ipn_postback: '.var_export($info,true)."\n");
  ...
  //use invoice to update order status
  $order_invoice = $_POST['invoice'];
  /**************************/
  if ($debug)fwrite($log_file, 'lookup data: '.var_export($lookupData,true)."\n");
  ...
  /**************************/
  if ($debug)fwrite($log_file, 'txn_type: '.var_export($txn_type,true)."\n");
...
//        $insert_id = $order->create($order_totals);
      	...
      	/**************************/
  		if ($debug)fwrite($log_file, 'order_id: '.var_export($_SESSION['order_id'],true)."\n");
...
// 		$insert_id = $_SESSION['order_id'];
        ...
        /**************************/
  		if ($debug)fwrite($log_file, 'ipn_create_order_array: '.var_export($sql_data_array,true)."\n");
...
/**************************/
  		if ($debug)fwrite($log_file, 'pp_list_id: '.var_export($pp_hist_id,true)."\n");     
  		if($debug)fclose($log_file);   
...
//         $_SESSION['order_number_created'] = $insert_id;
        ...
        require 'includes/languages/'.$_SESSION['language'].'/checkout_process.php';
