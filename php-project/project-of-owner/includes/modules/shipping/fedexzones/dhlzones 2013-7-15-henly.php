 ...
 * @version $Id: dhlzones.php 6347 2007-05-20 19:46:59Z ajeh $
 */
/*
...
//                $shipping_method = MODULE_SHIPPING_DHLZONES_TEXT_WAY . ' ' . $dest_country . (SHIPPING_BOX_WEIGHT_DISPLAY >= 2 ? ' : ' . $total_weight . ' ' . MODULE_SHIPPING_DHLZONES_TEXT_UNITS : '');
        ...
        if (strstr($zones_table[$i+1], '%')) {
          ...
          /*bof set the shipping cost for our own*/
        	if($i == 0 || $i == 2)
        	{
//        		echo 'yes';exit;
        		$shipping_array = explode('-',$zones_table[$i+1]);
        		$weight_count   = (!strpos(strval(($total_weight / 0.5 )),'.')) ?  intval(($total_weight / 0.5) -1) : (int)($total_weight / 0.5);
        		$shipping 		= (float)$shipping_array[0] + ($weight_count * (float)$shipping_array[1]);
        		/* echo $shipping_array[0] . ', ' . $shipping_array[1] . ', ' . $weight_count . ','.$shipping;exit; */
        	}else
        	{
        		$shipping = ($zones_table[$i+1] * $total_weight); 
        	}
        	
//        	exit($total_weight. ', '.$shipping);
        	/*eof set the shipping cost for our own*/
        }
                break;
        	    }
      	    break;
...
// shipping adjustment
        ...
        if ($shipping == -1) {
          ...
          switch (MODULE_SHIPPING_DHLZONES_METHOD) {
              ...
              $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_DHLZONES_HANDLING_' . $dest_zone);
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_DHLZONES_STATUS', 'MODULE_SHIPPING_DHLZONES_METHOD', 'MODULE_SHIPPING_DHLZONES_TAX_CLASS', 'MODULE_SHIPPING_DHLZONES_TAX_BASIS', 'MODULE_SHIPPING_DHLZONES_SORT_ORDER', 'MODULE_SHIPPING_DHLZONES_SKIPPED');
