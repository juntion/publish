...
class emszones
{
    ...
    {
        global $order, $shipping_weight, $shipping_num_boxes, $total_count, $currencies, $total_weight;

        //暂时屏蔽ems物流
        return array();

        $shipping_weight = $total_weight;
        $is_shipping_free = ($order->local_info['is_shipping_free'] || $order->delay_info['is_shipping_free']);
                    ...
                    case (MODULE_SHIPPING_EMSZONES_METHOD == 'Weight'):
                        ...
                        if ($shipping_weight <= $zones_table[$i]) {
                            ...
                            if ($shipping_weight <= 0.5) {
                                ...
                                $shipping = $zones_table[$i - 1] + ceil(($shipping_weight - 0.5) / 0.5) * $zones_table[$i + 1];
                            }
            ...
            if ($shipping == -1) {
                ...
                switch (MODULE_SHIPPING_EMSZONES_METHOD) {
                    case (MODULE_SHIPPING_EMSZONES_METHOD == 'Weight'):
                        ...
                        $num_arr = $order->get_order_num();
                        $num = $num_arr['num'];
                        if ($shipping && $num > 0) {
                            $shipping = $shipping + 250 * $num;
                        }
    ...
    function install()
    {
        ...
        $db->Execute("insert into configuration (configuration_title,configuration_key,configuration_value,configuration_description,configuration_group_id,sort_order,last_modified,date_added,use_function,set_function) values ('时间天数','MODULE_SHIPPING_EMSZONES_DAYS','3-7 Days','时间天数','6',0,'','2013-06-30 18:04:48','','')");
        for ($i = 1; $i <= $this->num_zones_days; $i++) {

            $db->Execute("insert into configuration (configuration_title,configuration_key,configuration_value,configuration_description,configuration_group_id,sort_order,last_modified,date_added,use_function,set_function) values ('Delivery_Time_Zone" . $i . "','MODULE_SHIPPING_EMSZONES_DAYS_ZONES_" . $i . "','US','countrys','6',0,'','2013-06-30 18:04:48','','zen_cfg_textarea(')");

            $db->Execute("insert into configuration (configuration_title,configuration_key,configuration_value,configuration_description,configuration_group_id,sort_order,last_modified,date_added,use_function,set_function) values ('Delivery_Time_" . $i . "','MODULE_SHIPPING_EMSZONES_DAYS_" . $i . "','3-7 Days','Delivery Time','6',0,'','2013-06-30 18:04:48','','')");
        }
    }
    ...
    function keys()
    {
        $keys = array('MODULE_SHIPPING_EMSZONES_STATUS', 'MODULE_SHIPPING_EMSZONES_METHOD', 'MODULE_SHIPPING_EMSZONES_TAX_CLASS', 'MODULE_SHIPPING_EMSZONES_TAX_BASIS', 'MODULE_SHIPPING_EMSZONES_SORT_ORDER', 'MODULE_SHIPPING_EMSZONES_DAYS', 'MODULE_SHIPPING_EMSZONES_SKIPPED');

        for ($i = 1; $i <= $this->num_zones_days; $i++) {
            $keys[] = 'MODULE_SHIPPING_EMSZONES_DAYS_ZONES_' . $i;
            $keys[] = 'MODULE_SHIPPING_EMSZONES_DAYS_' . $i;
        }
        for ($i = 1; $i <= $this->num_zones; $i++) {
