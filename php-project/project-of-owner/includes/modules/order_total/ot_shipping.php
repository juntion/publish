    ...
    function process() {
      global $order, $currencies;
        ...
        $this->output[] = array('title' => $this->title,
                                'text' => $currencies->total_format($order->info['shipping_cost'], true, $order->info['currency'], $order->info['currency_value']),
                                'value' => $order->info['shipping_cost']);
    }
    function process_new($set_info,$session_shipping="") {
      global $order, $currencies;
      if (MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING == 'true') {
        switch (MODULE_ORDER_TOTAL_SHIPPING_DESTINATION) {
          case 'national':
        ...
        if ( ($pass == true) && ( ($set_info['total'] - $set_info['shipping_cost']) >= MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER) ) {
          $set_info['shipping_method'] = $this->title;
          $set_info['total'] -= $set_info['shipping_cost'];
          $set_info['shipping_cost'] = 0;
        }
      }
      $module = substr($_SESSION['shipping']['id'], 0, strpos($_SESSION['shipping']['id'], '_'));
      if (zen_not_null($set_info['shipping_method'])) {
        if ($GLOBALS[$module]->tax_class > 0) {
          if (!isset($GLOBALS[$module]->tax_basis)) {
            $shipping_tax_basis = STORE_SHIPPING_TAX_BASIS;
          } else {
          ...
          $shipping_tax_amount = zen_calculate_tax($set_info['shipping_cost'], $shipping_tax);
          $set_info['shipping_tax'] += $shipping_tax_amount;
          $set_info['tax'] += $shipping_tax_amount;
          $set_info['tax_groups']["$shipping_tax_description"] += zen_calculate_tax($set_info['shipping_cost'], $shipping_tax);
          $set_info['total'] += zen_calculate_tax($set_info['shipping_cost'], $shipping_tax);
          $_SESSION['shipping_tax_description'] =  $shipping_tax_description;
          $_SESSION['shipping_tax_amount'] =  $shipping_tax_amount;
          if (DISPLAY_PRICE_WITH_TAX == 'true') $set_info['shipping_cost'] += zen_calculate_tax($set_info['shipping_cost'], $shipping_tax);
        }

        if ($session_shipping == 'free_free') {
          $set_info['shipping_method'] = FREE_SHIPPING_TITLE;
          $set_info['shipping_cost'] = 0;
        }

      }
      $this->output_new[] = array('title' => $this->title,
          'text' => $currencies->total_format($set_info['shipping_cost'], true, $set_info['currency'], $set_info['currency_value']),
          'value' => $set_info['shipping_cost']);
    }
    function check() {
	  global $db;
