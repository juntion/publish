    ...
    function process() {
        ...
        if (SHOW_SPLIT_TAX_CHECKOUT == 'true')
        {
          if ($value > 0 or ($value == 0 && STORE_TAX_DISPLAY_STATUS == 1 )) {
            $this->output[] = array('title' => ((is_numeric($key) && $key == 0) ? TEXT_UNKNOWN_TAX_RATE :  $key) . ':',
                                    'text' => $currencies->total_format($value, true, $order->info['currency'], $order->info['currency_value']),
                                    'value' => $value);
          }
        } else
        {
      ...
      if (SHOW_SPLIT_TAX_CHECKOUT != 'true' && ($taxValue > 0 or STORE_TAX_DISPLAY_STATUS == 1))
      {
        $this->output[] = array(
                        ...
                        'text' => $currencies->total_format($taxValue, true, $order->info['currency'], $order->info['currency_value']) ,
                        'value' => $taxValue);
      }
    }
    function process_new($set_info,$session_shipping="") {
      ...
      if (STORE_TAX_DISPLAY_STATUS == 1)
      {
        ...
        if (count($result) > 0)
        {
          foreach ($result as $description)
          {
            if (!isset($set_info['tax_groups'][$description]))
            {
              $set_info['tax_groups'][$description] = 0;
            }
          }
        }
      }
      if (count($set_info['tax_groups']) > 1 && isset($set_info['tax_groups'][0])) unset($set_info['tax_groups'][0]);
      while (list($key, $value) = each($set_info['tax_groups'])) {
        if (SHOW_SPLIT_TAX_CHECKOUT == 'true')
        {
          if ($value > 0 or ($value == 0 && STORE_TAX_DISPLAY_STATUS == 1 )) {
            $this->output_new[] = array('title' => ((is_numeric($key) && $key == 0) ? TEXT_UNKNOWN_TAX_RATE :  $key) . ':',
                'text' => $currencies->total_format($value, true, $set_info['currency'], $set_info['currency_value']),
                'value' => $value);
          }
        } else
        {
      ...
      if (SHOW_SPLIT_TAX_CHECKOUT != 'true' && ($taxValue > 0 or STORE_TAX_DISPLAY_STATUS == 1))
      {
        $this->output_new[] = array(
            'title' => substr($taxDescription, 0 , strlen($taxDescription)-3) . ':' ,
            'text' => $currencies->total_format($taxValue, true, $set_info['currency'], $set_info['currency_value']) ,
            'value' => $taxValue);
      }
    }
    function check() {
