...
if (isset($_GET['ajax_request_action']) && $_GET['ajax_request_action']){
	...
	if(!zen_not_null($action)){
		...
		switch($_GET['ajax_request_action']){
					...
					if(!empty($_POST['Gplus_Id'])){
						
						 $google_plus_id = zen_db_prepare_input($_POST['Gplus_Id']);
						 $first_name = zen_db_prepare_input($_POST['fName']);
						 $last_name = zen_db_prepare_input($_POST['lName']);
						 ...
						 $locale = zen_db_prepare_input($_POST['Location']);
						 echo '{"fname":"'.$first_name.'","lname":"'.$last_name.'","name":"'.$name.'"}';
						 die($name);
...
//						}
				        ...
				        $text_message = 'New message from regist page of  FiberStore !';
				    		...
				    		<tr><td>
								...
								<tbody>
						    		<tr>
									    ...
									    <b style=" display:block; padding-bottom:10px;"><br><br>Dear '.$name.' ,</b>
