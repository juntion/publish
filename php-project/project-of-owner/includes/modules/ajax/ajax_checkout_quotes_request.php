...
$quotes_service = new QuoteService();
...
if (in_array($action, ['delete_address', 'add_customer_address', 'edit_shipping_address', 'edit_billing_address', 'confirm_shipping_address', 'confirm_billing_address'])) {
...
switch ($action) {
    ...
    case "add_customer_address":
        ...
        if ($_SESSION['sendto_quotes'] && $address_type == 2) {
        ...
        if ($_SESSION['sendto_quotes'] && $address_type == 0) {
            //验证地址
        ...
        $is_avaTaxCheck = $_SESSION['useUpsDefaultAddress_quotes'] == 1 ? false : true;
        ...
        if ($address_book_id) {
            ...
            $data['address_book_id'] = $address_book_id;
            if ($address_type == 0) {
                $_SESSION['sendto_quotes'] = $address_book_id;
                if ($isUseShipping == 1) {
                    $_SESSION['billto_quotes'] = $_SESSION['sendto_quotes'];
                }
                if ($inquiry_id) {
                    ...
                    $order = new order_quotes("", $cart_products);
                } else {
            ...
            if (isset($_SESSION['checkout_default_ads_quotes'])) {
                unset($_SESSION['checkout_default_ads_quotes']);
            }
            $api->setStatus(200)->response($data);
        } else {
            $api->setStatus(406)->setMessage(FS_SYSTME_BUSY)->response();
    ...
    case 'confirm_shipping_address':
        ...
        $_SESSION['sendto_quotes'] = $address_book_id;
        if ($isUseShippingAddressAsBilling) {
            $_SESSION['billto_quotes'] = $address_book_id;
        }
        if ($inquiry_id) {
            require_once(DIR_WS_CLASSES . 'inquiry.class.php'); //类或者方法
            $inquiry = new inquiry($currencies);
        ...
        $data['info']['price_data']['vat_title'] = get_checkout_vat_title($order->delivery['country']['iso_code_2'],
    ...
    case 'confirm_billing_address':
        ...
        $current_shipping_id = $_SESSION['sendto_quotes'];
        $current_billing_id = $_SESSION['billto_quotes'];
        ...
        if ($is_use == 1) {
            ...
            $_SESSION['billto_quotes'] = $shipping_address_id;
            ...
            if (!empty($is_valid)) {
                ...
                if (!empty($is_valid['entry_postcode']['zip_valid'])) {
                    ...
                    $return_data = array(
                        ...
                        'erro' => $is_valid['tax_number']['remote_validate_tax_number']
                    );
                    $api->setStatus(422)->response($return_data);
                } else {
            ...
            $_SESSION['billto_quotes'] = $billing_address_id;
        }
        ...
        if ($current_shipping_id) {
            if (!$inquiry_id) {
                ...
                if ($cart_products == false) {
                    $_SESSION['billto_quotes'] = $current_billing_id;
                    $api->setStatus(406)->setMessage(FS_INQUIRY_INFO_66)->response();
                }
                if ($cart_products == false) {
                    $order = new order_quotes();
            ...
            $local_shipping_method = $order->handleShippingMethod($_SESSION['shipping_local_quotes']['id']);
            $delay_shipping_method = $order->handleShippingMethod($_SESSION['shipping_delay_quotes']['id']);
            $global_shipping_method = $order->handleShippingMethod($_SESSION['shipping_global_quotes']['id']);
        ...
        $data['address_book_id'] = $is_use ? $current_shipping_id : $_SESSION['billto_quotes'];
            ...
            if ('no' == zen_db_prepare_input($_POST['fileExists'])) {
                ...
                if (Checkout::fileExists()) {//有文件
                    ...
                    if (!empty(zen_db_prepare_input($_POST['cardPath']))) {
                        ...
                        $ru_service->createRuOrderInfo($insertData);
                        $resData = $ru_service->lastPaymentInformation();
                        $_SESSION['payment_quotes'] = $payment_method;
                        $api->setStatus(200)->response($resData);
                    } else {
                        $api->setStatus(406)->setMessage(FS_CHECKOUT_RU_FILE_TIPS_2)->response();
                    }
    ...
    case "change_shipping":
        ...
        if (sizeof($shipping_cost)) {
            foreach ($shipping_cost as $v) {
                if ($v['methods'] == $shipping_method) {
                    $_SESSION['shipping_' . $shipping_type . '_quotes'] = array(
                        
                        'id' => $v['methods'] . '_' . $v['methods'],
        ...
        $shipping_address = $customer_info->get_select_address($_SESSION['sendto_quotes']);
        $checkout = Checkout::getInstance();
        $is_shipping_free = false;
        $symbol = $currencies->currencies[$_SESSION['currency']]['symbol_left'];
        $symbol_right = $currencies->currencies[$_SESSION['currency']]['symbol_right'];
        ...
        foreach ($shipping_arr as $kk) {
            if ($_SESSION['shipping_' . $kk . '_quotes']) {
                $origin_shipping_cost += $_SESSION['shipping_' . $kk . '_quotes']['origin_price'];
            } else {
                unset($_SESSION['shipping_' . $kk . '_quotes']);
            }
        }
    ...
    case 'create_order':
        ...
        //如果缺失运输地址或者账单地址
        if(empty($_SESSION['sendto_quotes'])|| empty($_SESSION['billto_quotes'])){
            $api->setStatus(406)->setMessage(FS_SYSTME_BUSY)->response();
            return;
        }
        ...
        $_SESSION['delivery_array_quotes'] = zen_db_prepare_input($_POST['tikectInfo']);
        require_once(DIR_WS_CLASSES . 'shipping.php');
        require_once(DIR_WS_CLASSES . 'payment.php');
        $payment = new payment();
        require_once(DIR_WS_CLASSES . 'order_quotes.php');
        ...
        if ($inquiry_id) {
            ...
            $order = new order_quotes("", $cart_products, $senTo, $billTo);/*for paypal load shipping */
        } else {
            $order = new order_quotes("", "", $senTo, $billTo);/*for paypal load shipping */
        }
        ...
        $_SESSION['payment_quotes'] = isset($_SESSION['payment_quotes']) ? $_SESSION['payment_quotes'] : 'paypal';
        $_SESSION['payment_quotes'] = $_SESSION['payment_quotes'] ? $_SESSION['payment_quotes'] : 'paypal';
        $payment = new payment($_SESSION['payment_quotes']);
        if ($debug) fwrite($handle, ' init payment - ' . time() . " \n");
        if ($_SESSION['cart']->get_checked_products()['checkedProducts'] || $inquiry_id) {
            ...
            if (!empty($related_email)) {
                $_SESSION['related_email_quotes'] = $related_email;
            }
            if (!empty($customers_po)) {
                $_SESSION['customers_po_quotes'] = $customers_po;
            }
            $customer_remarks = zen_db_input($_POST['customer_remarks']);
            if (!empty($customer_remarks)) {
                $_SESSION['customer_remarks_quotes'] = $customer_remarks;
            }
            ...
            //订单生成后及时删除运输信息
            if (isset($_SESSION['related_email_quotes'])) {
                unset($_SESSION['related_email_quotes']);
            }
            //订单生成后及时删除客户remark信息
            if (isset($_SESSION['customer_remarks_quotes'])) {
                unset($_SESSION['customer_remarks_quotes']);
            }
            if (isset($_SESSION['customers_po_quotes'])) {
                unset($_SESSION['customers_po_quotes']);
            }

            switch (true) {
                case 'paypal' == $_SESSION['payment_quotes']:
                    ...
                    if (isset($_SESSION['shipping_delay_quotes'])) {
                        unset($_SESSION['shipping_delay_quotes']);
                    }
                    if (isset($_SESSION['shipping_local_quotes'])) {
                        unset($_SESSION['shipping_local_quotes']);
                    }
                    if (isset($_SESSION['useUpsDefaultAddress_quotes'])) {
                        unset($_SESSION['useUpsDefaultAddress_quotes']);
                    }
                    $data = array(
                        'type' => $_SESSION['payment_quotes'],
                        'url' => $action,
                    ...
                    $api->setStatus(200)->response($data);
                    break;
                case  in_array($_SESSION['payment_quotes'], array('bpay', 'eNETS', 'iDEAL', 'SOFORT', 'YANDEX', 'WEBMONEY')):

                    $order = new order_quotes($order_id);
                    $class = &$_SESSION['payment_quotes'];
                    $action = $GLOBALS[$class]->form_action_url;
                    if ($debug) fwrite($handle, 'action url: ' . $action . ' - ' . time() . " \n");
                    if (isset($_SESSION['shipping_delay_quotes'])) {
                ...
                case  ('globalcollect' == $_SESSION['payment_quotes'] || 'payeezy' == $_SESSION['payment_quotes']):
                    unset($_SESSION['sendto_quotes']);
                    unset($_SESSION['billto_quotes']);
                    if (isset($_SESSION['shipping_delay_quotes'])) {
                        unset($_SESSION['shipping_delay_quotes']);
                    }
                    if (isset($_SESSION['shipping_local_quotes'])) {
                    ...
                    unset($_SESSION['payment_quotes']);
                    unset($_SESSION['comments_quotes']);
            ...
            if ('paypal' == $_SESSION['payment_quotes']) {
                $api->setStatus(406)->setMessage(FS_SYSTME_BUSY)->response();
            }
        }
    ...
    case 'save_customer_po':
        ...
        $_SESSION['customers_po_quotes'] = zen_db_input($_POST['poNumber']);
        $_SESSION['customer_remarks_quotes'] = $_POST['customer_remarks'];
        $_SESSION['products_custom_quotes'] = $_POST['products_custom'];
        $_SESSION['need_invoice_quotes'] = $_POST['invoice_need'];
        ...
        if(!empty($_POST['comments'])){
            $_SESSION['remarks_quotes'] = zen_db_prepare_input($_POST['comments']);
        }
        $post_cart_counts = (int)$_POST['cart_counts'];
        $total_count = $_SESSION['cart']->count_contents(true);
        if ($post_cart_counts != $total_count) {
        ...
        if ($_SESSION['payment_quotes'] == "alfa" && !$alfa_id) {
            $api->setStatus(406)->setMessage(FS_SHOPPING_CAUTION)->response();
        }
        $_SESSION['alfa_info_quotes'] = $alfa_id;

        if ($installTime) {
            $_SESSION['install_quotes'] = [
                'localInstall' => $localInstall,
                'installTime' => $installTime
            ];
        }
        ...
        createOrderShippingInfo($delivery_data);
        if (!empty($_SESSION['payment_quotes']) && $_SESSION['payment_quotes'] == "echeck") {
            $_SESSION['echeck_info_quotes'] = zen_db_prepare_input($_POST['echeckInfo']);
        }

        $api->setStatus(200)->response($_SESSION['alfa_info_quotes']);
        break;
    ...
    case 'send_email':
        ...
        if (isset($orders_id) && zen_check_order_exist($orders_id)) {
            ...
            require_once(DIR_WS_CLASSES . 'order_quotes.php');
            $order = new order_quotes($orders_id);/*for paypal load shipping */
