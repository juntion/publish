 ...
 */
  ...
  function zen_exist_invoice($order_invoice){
  	global $db;
  	$get_orders_id = $db->Execute("select count(orders_id) as total from " . TABLE_ORDERS . " where orders_number = '" . $order_invoice ."'");
  	if ($get_orders_id->fields['total']) return 1;
  	return 0;
  }
  function zen_get_orders_id_by_invoice($order_invoice){
  	global $db;
  	$get_orders_id = $db->Execute("select orders_id from " . TABLE_ORDERS . " where orders_number = '" . $order_invoice ."'");
  	return $get_orders_id->fields['orders_id'];
  }
 ...
 */
  ...
  function select_pp_currency() {
    ...
    $pp_currencies = array('CAD', 'EUR', 'GBP', 'JPY', 'USD', 'AUD', 'CHF', 'CZK', 'DKK', 'HKD', 'HUF', 'NOK', 'NZD', 'PLN', 'SEK', 'SGD', 'THB', 'MXN', 'ILS', 'PHP', 'TWD', 'BRL', 'MYR','RUB');
  ...
  function valid_payment($amount, $currency, $mode = 'IPN') {
    ...
    $transaction_amount = preg_replace('/[^0-9.]/', '', round($exchanged_amount,2));
 ...
 */
  ...
  function ipn_getLineItemDetails() {
    ...
    for ($i=0, $n=sizeof($order->products), $k=1; $i<$n; $i++, $k++) {
      ...
      if ($order->products[$i]['paypal_price'] == 0) continue;
      ...
      $m1 = zen_round($order->products[$i]['qty'] * $order->products[$i]['paypal_price'], $currencies->currencies[$_SESSION['currency']]['decimal_places']);
      $m2 = ($order->products[$i]['qty'] * zen_round($order->products[$i]['paypal_price'], $currencies->currencies[$_SESSION['currency']]['decimal_places']));
      $n1 = $order->products[$i]['qty'] * zen_calculate_tax($order->products[$i]['paypal_price'], $order->products[$i]['tax']);
      $n2 = zen_calculate_tax($order->products[$i]['qty'] * $order->products[$i]['paypal_price'], $order->products[$i]['tax']);
      ...
      if ($flag_treat_as_partial || $order->products[$i]['qty'] != (int)$order->products[$i]['qty']) {
		...
		$optionsLI["amount_$k"] = round($order->products[$i]['qty'] * $order->products[$i]['paypal_price'],2);
        $optionsLI["tax_$k"] = zen_calculate_tax(zen_round($order->products[$i]['qty'] * $order->products[$i]['paypal_price'], $currencies->currencies[$_SESSION['currency']]['decimal_places']), $order->products[$i]['tax']);
        $optionsLI["quantity_$k"] = 1;
      } else {
        $optionsLI["amount_$k"] = $order->products[$i]['paypal_price'];
        $optionsLI["quantity_$k"] = $order->products[$i]['qty'];
        $optionsLI["tax_$k"] = zen_calculate_tax(1 * $order->products[$i]['paypal_price'], $order->products[$i]['tax']);
      }
 ...
 */
    ...
    if (!defined('PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING') || PAYPALWPP_SKIP_LINE_ITEM_DETAIL_FORMATTING != 'true' || in_array($order->info['currency'], array('JPY', 'NOK'))) {
      if (is_array($optionsST)) foreach ($optionsST as $key=>$value) {
        $optionsST[$key] = round($value, ($order->info['currency'] == 'JPY' ? 0 : 2));
      }
      if (is_array($optionsLI)) foreach ($optionsLI as $key=>$value) {
        if (substr($key, 0, 8) == 'tax_' && ($optionsLI[$key] == '' || $optionsLI[$key] == 0)) {
          unset($optionsLI[$key]);
        } else {
          if (strstr($key, 'amount')) $optionsLI[$key] = round($value, ($order->info['currency'] == 'JPY' ? 0 : 2));
        }
      }
    }

    ipn_logging('getLineItemDetails 7', 'checking subtotals... ' . "\n" . print_r(array_merge(array('calculated total'=>round($stAll, ($order->info['currency'] == 'JPY' ? 0 : 2))), $optionsST), true) . "\n-------------------\ndifference: " . ($stDiff + 0) . '  (abs+rounded: ' . ($stDiffRounded + 0) . ')');
