...
function update_status() {
  ...
  function admin_notification($zf_order_id) {
    global $db;
    $output = '';
    $sql = "select * from " . TABLE_WESTUNION_ORDERS . " as o left join ".TABLE_COUNTRIES ." as c on o.sender_country_id = c.countries_id where o.orders_id = '" . (int)$zf_order_id . "' order by westerunion_id  ";
    $westernuion = $db->Execute($sql);
    if ($westernuion->RecordCount() > 0 && file_exists(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/westernunion/westernunion_admin_notification.php')) require(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/westernunion/westernunion_admin_notification.php');
    return $output;
  }
  }
