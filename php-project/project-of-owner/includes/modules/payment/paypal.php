...
//                   'lc' => $order->customer['country']['iso_code_2'],
                   ...
                   'cancel_return' => zen_href_link(FILENAME_MANAGE_ORDERS, '', 'SSL'),
...
//      $optionsLineItems['num_cart_items'] = sizeof($order->products);
    ...
    $optionsAggregate = array(
                   ...
                   'amount' => round($this->transaction_amount,2),
                   'shipping' => '0.00',
                    );
...
function process_string() {
    ...
    if ($telephone != '') {
      $optionsPhone['H_PhoneNumber'] = $telephone;
      if (in_array($order->customer['countries_iso_code_2'], array('US','CA'))) {
        $optionsPhone['night_phone_a'] = substr($telephone,0,3);
        $optionsPhone['night_phone_b'] = substr($telephone,3,3);
        $optionsPhone['night_phone_c'] = substr($telephone,6,4);
        $optionsPhone['day_phone_a'] = substr($telephone,0,3);
    ...
    $optionsCust = array(
                   ...
                   'address1' => replace_accents($order->customer['street_address']),
                   'city' => replace_accents($order->customer['city']),
                   'state' =>  replace_accents($order->customer['state']),
                   'zip' => $order->customer['postcode'],
                   'country' => $order->customer['countries_iso_code_2'],
                   'email' => $order->customer['email_address'],
                   );
    // address line 2 is optional
    if ($order->customer['suburb'] != '') $optionsCust['address2'] = $order->customer['suburb'];
    // different format for Japanese address layout:
    if ($order->customer['countries_iso_code_2'] == 'JP') $optionsCust['zip'] = substr($order->customer['postcode'], 0, 3) . '-' . substr($order->customer['postcode'], 3);
    if (MODULE_PAYMENT_PAYPAL_ADDRESS_REQUIRED == 2) {
      $optionsCust = array(
                   'first_name' => replace_accents($order->delivery['firstname'] != '' ? $order->delivery['firstname'] : $order->billing['firstname']),
                   'last_name' => replace_accents($order->delivery['lastname'] != '' ? $order->delivery['lastname'] : $order->billing['lastname']),
                   'address1' => replace_accents($order->delivery['street_address'] != '' ? $order->delivery['street_address'] : $order->billing['street_address']),
                   'city' => replace_accents($order->delivery['city'] != '' ? $order->delivery['city'] : $order->billing['city']),
                   'state' => replace_accents($order->delivery['state'] != '' ? $order->delivery['state'] : $order->billing['state']),
                   'zip' => ($order->delivery['postcode'] != '' ? $order->delivery['postcode'] : $order->billing['postcode']),

				   'country' => ($order->delivery['countries_iso_code_2'] != '' ? $order->delivery['countries_iso_code_2'] : $order->billing['countries_iso_code_2']),

                   'email' => $order->customer['email_address'],
                   );
      if ($order->delivery['suburb'] != '') $optionsCust['address2'] = $order->delivery['suburb'];
      if ($order->delivery['countries_iso_code_2'] == 'JP') $optionsCust['zip'] = substr($order->delivery['postcode'], 0, 3) . '-' . substr($order->delivery['postcode'], 3);
    }
    $optionsShip['no_shipping'] = MODULE_PAYMENT_PAYPAL_ADDRESS_REQUIRED;
    if (MODULE_PAYMENT_PAYPAL_ADDRESS_OVERRIDE == '1') $optionsShip['address_override'] = MODULE_PAYMENT_PAYPAL_ADDRESS_OVERRIDE;
    // prepare cart contents details where possible
...
//    $process_button_string = implode("\n", $buttonArray) . "\n";
  ...
  function process_account_paypal_submit() {
  	global $db, $order, $currencies, $currency;
  	$options = array();
  	$optionsCore = array();
  	$optionsPhone = array();
	...
	$this->transaction_amount = $this->totalsum * $order->info['currency_value'];
  	...
  	if ($order->customer['suburb'] != '') $optionsCust['address2'] = $order->customer['suburb'];
  	// different format for Japanese address layout:
  	if (isset($order->customer['countries_iso_code_2']) && $order->customer['countries_iso_code_2'] == 'JP') $optionsCust['zip'] = substr($order->customer['postcode'], 0, 3) . '-' . substr($order->customer['postcode'], 3);
  	if (MODULE_PAYMENT_PAYPAL_ADDRESS_REQUIRED == 2) {
  		$optionsCust = array(
  				'first_name' => replace_accents($order->delivery['firstname'] != '' ? $order->delivery['firstname'] : $order->billing['firstname']),
  				'last_name' => replace_accents($order->delivery['lastname'] != '' ? $order->delivery['lastname'] : $order->billing['lastname']),
				...
				'state' => replace_accents($order->delivery['state'] != '' ? $order->delivery['state'] : $order->billing['state']),
  				'zip' => $order->delivery['postcode'] != '' ? $order->delivery['postcode'] : $order->billing['postcode'],
  				/* 'country' => ($order->delivery['country'] ? $order->delivery['country'] : $order->billing['country']) ($order->delivery['country']['title'] != '' ? $order->delivery['country']['title'] : $order->billing['country']['title']),
  				'country_code' => ''*/
				'country' => $order->delivery['country'] != '' ? get_countries_code($order->delivery['country']) : get_countries_code($order->billing['country']),
  				'email' => $order->customer['email_address'],
  		);
  		if ($order->delivery['suburb'] != '') $optionsCust['address2'] = $order->delivery['suburb'];
  		if (isset($order->delivery['countries_iso_code_2']) && $order->delivery['countries_iso_code_2'] == 'JP') $optionsCust['zip'] = substr($order->delivery['postcode'], 0, 3) . '-' . substr($order->delivery['postcode'], 3);
  	}
  	$optionsShip['no_shipping'] = MODULE_PAYMENT_PAYPAL_ADDRESS_REQUIRED;
  	if (MODULE_PAYMENT_PAYPAL_ADDRESS_OVERRIDE == '1') $optionsShip['address_override'] = MODULE_PAYMENT_PAYPAL_ADDRESS_OVERRIDE;
  	// prepare cart contents details where possible
  ...
  function getLanguageCode() {
    global $order;
    $lang_code = '';
    if (isset($order->customer['country']['id']) && $order->customer['country']['id']){
  ...
  function before_process() {
    ...
    unset($_SESSION['paypal_transaction_info'],$_SESSION['order_id']);
    if (isset($_GET['referer']) && $_GET['referer'] == 'paypal') {
      ...
      if (defined('MODULE_PAYMENT_PAYPAL_PDTTOKEN') && MODULE_PAYMENT_PAYPAL_PDTTOKEN != '' && isset($_GET['tx']) && $_GET['tx'] != '') {
      	$_SESSION['order_summary']['tx'] = $_GET['tx'];
        $pdtStatus = $this->_getPDTresults($this->transaction_amount, $this->transaction_currency, $_GET['tx']);
      } else {
        $pdtStatus = false;
      }
      $order_invoice = $this->pdtData['invoice'];
      if ($order_invoice && zen_exist_invoice($order_invoice)){
      	$insert_id = zen_get_orders_id_by_invoice($order_invoice);
      ...
      /******************************/
      //$log_file = @fopen('ipn.log.txt', 'a+');
      //fwrite($log_file, '$pdt status: '.($pdtStatus ? ' true' : ' false') ."\n");fclose($log_file);
      if ($pdtStatus == false) {
      ...
      zen_redirect(zen_href_link(FILENAME_CHECKOUT_SUCCESS, '&type=paypal', 'SSL'));
    }
  }
  /**
    * Checks referrer
  ...
  function after_process() {
    ...
    // PDT found order to be approved, so add a new OSH record for this order's PP details
      ...
      if ($order_invoice && zen_exist_invoice($order_invoice)){
      	  $insert_id = zen_get_orders_id_by_invoice($order_invoice);
//          $flag = set_order_key_for_payment($insert_id);
...
//      	  $db->Execute($sql);
	      $sql_data_array= array(array('fieldName'=>'orders_id', 'value'=>$insert_id, 'type'=>'integer'),
	                             array('fieldName'=>'orders_status_id', 'value'=>$new_status, 'type'=>'integer'),
          ...
          $oid_arr = zen_get_all_son_order_id($insert_id);
          foreach($oid_arr as $v){
              $db->query("update orders set orders_status = '2' where orders_id = '".$v."'");
          ...
          if ($order->info['orders_status_id'] == 5){
              ...
              $son_id_str = '(' . implode(',', $oid_arr) . ')';
              //$ordersOvertime =  fs_get_one_data(TABLE_ORDERS_OVERTIME, " type=1 and orders_id=" . $insert_id,"id");
	...
	//TODO send order mail
	      $order->send_fs_order_email($insert_id,true);
      }
      return $notice;
    }
  }
...
// sandbox:  www.sandbox.paypal.com/cgi-bin/webscr
  ...
  function get_country_id_by_country_name($country_name){
  	global $db;
  	$sql = "SELECT countries_id FROM countries WHERE countries_name = :countries_name:;";
  	$sql = $db->bindVars($sql,':countries_name:',$country_name,'string');
  	$result= $db->Execute($sql); 
  	return $result->fields['countries_id'];
  }
}
