 ...
 * @version $Id: paypalwpp.php 17993 2016-11-19 02:41:28Z HENLY $
 */
/**
 * 
 */
...
class paypalwpp extends base {
  ...
  function paypalwpp() {
    ...
    $this->buttonSourceEC = 'FS-EC_us';
    $this->buttonSourceDP = 'FS-DP_us';
    if (MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'Payflow-UK') {
      $this->buttonSourceEC = 'FS-EC_uk';
      $this->buttonSourceDP = 'FS-DP_uk';
    }
    if (MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'Payflow-US') {
      $this->buttonSourceEC = 'FS-ECGW_us';
      $this->buttonSourceDP = 'FS-GW_us';
    }
    ...
    if (!$this->in_special_checkout()) {
      $this->form_action_url = zen_href_link('paypal_express.php', 'type=ec&markflow=1&clearSess=1&stage=final', 'SSL', true, true, true);
    }
  ...
  function before_process() {
    global $order, $doPayPal, $messageStack,$orderNmber;
    $options = array();
    $optionsShip = array();
    $optionsNVP = array();
  ...
  function after_process() {
	...
	if(strpos($this->payment_status,'Pending') === false){
		$pay_status = 2;
	}else{
		$pay_status = 1;
	}
    ...
    $sql_data_array= array(array('fieldName'=>'orders_id', 'value'=>$insert_id, 'type'=>'integer'),
                           array('fieldName'=>'orders_status_id', 'value'=>$pay_status, 'type'=>'integer'),
    ...
    $paypal_order = array('order_id' => $insert_id,
                          ...
                          'invoice' => urldecode($this->responsedata['PPREF']),
...
//     }
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('PAYFLOW: Partner', 'MODULE_PAYMENT_PAYPALWPP_PFPARTNER', 'FS', 'Your Payflow Partner name linked to your Payflow account. This value is case-sensitive.<br />Typical values: <strong>PayPal</strong> or <strong>FS</strong>', '6', '25', now())");
  ...
  function alterShippingEditButton() {
    ...
    if ($this->in_special_checkout() && empty($_SESSION['paypal_ec_markflow'])) {
      return zen_href_link('paypal_express.php', 'type=ec&clearSess=1', 'SSL', true,true, true);
    }
  }
  /**
 ...
 */
  ...
  function ec_step1() {
    ...
    $return_url = str_replace('&amp;', '&', zen_href_link('paypal_express.php', 'type=ec', 'SSL', true, true, true));
    // Select the return URL if they click "cancel" on PayPal site or click to return without making payment or login
    $cancel_url = str_replace('&amp;', '&', zen_href_link(($_SESSION['customer_first_name'] != '' && $_SESSION['customer_id'] != '' ? FILENAME_SHOPPING_CART :FILENAME_SHOPPING_CART), 'ec_cancel=1', 'SSL'));
...
//echo '<br>3rd submission. {'.$response['L_ERRORCODE0'].'}<pre>'.print_r($options, true);
  ...
  function ec_step2() {
    ...
    if (MODULE_PAYMENT_PAYPALWPP_CONFIRMED_ADDRESS == 'Yes' && strtoupper($response['ADDRESSSTATUS']) != 'CONFIRMED') {
      $this->terminateEC(MODULE_PAYMENT_PAYPALWPP_TEXT_CONFIRMEDADDR_ERROR, true, FILENAME_SHOPPING_CART);
    }
...
//    }
    ...
    $this->ec_step2_finish(array_merge($step2_payerinfo, $step2_shipto), $this->new_acct_notify,$order);
  }

  /**
   * Complete the step2 phase by creating accounts if needed, linking data, placing order, etc.
   */
  function ec_step2_finish($paypal_ec_payer_info, $new_acct_notify,$order) {
    global  $db;
    ...
    if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id']) && $_SESSION['customer_id'] > 0) {
      ...
      if ($_SESSION['paypal_ec_markflow'] == 1) {
		...
		$this->terminateEC('', false, 'checkout_paypal_confirm');
      }
    } else {
      ...
      $this->user_login($_SESSION['paypal_ec_payer_info']['payer_email'], $order,false);
...
///        $fromDb = str_replace("\n", '', $answers_arr->fields['entry_firstname'].$answers_arr->fields['entry_lastname']);
   ...
   */
  function user_login($email_address,$order, $redirect = true) {
    global $db, $messageStack;
    ...
    if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id'])) {
      if ($this->showPaymentPage === true || $goto_page == FILENAME_CHECKOUT_PAYMENT) {
        ...
        if ((!$_SESSION['shipping'] || $_SESSION['shipping'] == '') || $_SESSION['shipping']['cost'] < 0) {
          // debug
          $this->zcLog('termEC-5', 'Have no shipping method selected, or shipping < 0 so set FILENAME_SHOPPING_CART');
          $redirect_path = FILENAME_SHOPPING_CART;
          $stackAlert = FILENAME_SHOPPING_CART;
        } else {
          // debug
		  ...
		  $redirect_path = 'checkout_paypal_confirm';
		  $stackAlert = 'checkout_paypal_confirm';
        }
      } elseif ($goto_page) {
        // debug
		...
		if ($goto_page == FILENAME_SHOPPING_CART) $stackAlert = 'checkout_paypal_confirm';
      } else {
        // debug
      ...
      $_SESSION['navigation']->set_snapshot(FILENAME_SHOPPING_CART);
