...
function zen_nonssl_redirect($url, $httpResponseCode = '') {
    global $request_type;
    if ( (ENABLE_SSL == true) && ($request_type == 'SSL') ) {
        if (substr($url, 0, strlen(HTTP_SERVER . DIR_WS_CATALOG)) == HTTP_SERVER . DIR_WS_CATALOG) {
            $url = HTTP_SERVER . DIR_WS_CATALOG . substr($url, strlen(HTTP_SERVER . DIR_WS_CATALOG));
        }
    }
    while (strstr($url, '&&')) $url = str_replace('&&', '&', $url);
    while (strstr($url, '&amp;&amp;')) $url = str_replace('&amp;&amp;', '&amp;', $url);
...
* 公用函数  返回数据行数
* */
function fs_get_total_from_db($table,$where = '1',$oderby = ''){
    global $db;
    $sql="select count(*) AS total from ".$table."  where ".$where." $oderby ";
...
/*
 * 公用函数  返回数据表单个字段
 * $columns 为单个字段  如  products_id ，其他和上个函数一样
 * */
function fs_get_data_from_db_fields($columns,$table,$where,$oderby = ''){
    global $db;
 ...
 * */
function fs_get_data_from_db_fields_array($fields_array,$table,$where,$oderby=""){
    global $db;
    $returns = array();
    $field = implode(",",$fields_array);
    $sql="select ".$field." from ".$table."  where ".$where." $oderby ";
 ...
 * @return array
 */
function  fs_get_one_data($table,$where,$fields = "*"){
    global $db;
    $returns = [];
    ...
    if (!$result->EOF){
        if ($fields == "*"){
            ...
            foreach ($field as $column){
                $returns[$column] = isset($result->fields[$column]) ? $result->fields[$column] : '';
            }
        }
    }
    return $returns;
 ...
 * @return array[array]
 */
function  fs_get_datas($table,$where,$fields = "*"){
    global $db;
    $returns = [];
    ...
    if (!$result->EOF){
        if ($fields == "*"){
            ...
            while (!$result->EOF){
                $return = [];
                foreach ($field as $column){
                    $return[$column] = isset($result->fields[$column]) ? $result->fields[$column] : '';
                }
                $returns[] = $return;
                $result->MoveNext();
            }
...
function zen_sanitize_string($string) {
    ...
    if(is_object($db)){
        $string = $db->prepare_input($string);
    }else{
...
function zen_get_all_get_params($exclude_array = '', $search_engine_safe = true) {
    ...
    //part 1
    //bof hide patch cable narrow by
    $narrow_by_hidding = false;
    $target_categories = array(897,898,1120);
    $value = array();
    $display_css_class = 'sidebar_06_js';
    if (1 < sizeof($cPath_array) && in_array($cPath_array[1], $target_categories)) {
    ...
    if (is_array($_GET) && (sizeof($_GET) > 0)) {
        ...
        while (list($key, $value) = each($_GET)) {
            ...
            //if this is array
            if ('narrow' == $key && is_array($value) && sizeof($_GET['narrow'])) {
                array_unique($value);
...
WHERE products_narrow_by_options_values_id = " .(int)$v;
                        ...
                        if (!in_array($result[0][0],array(337,340,339,345,341,342,343))) {
                            $get_url.= '&'.$key.'[]='.$v.'&';
                        }
                    } else{
                        $get_url.= '&'.$key.'[]='.$v.'&';
            ...
            if ( (count($value) > 0) && ($key != 'main_page') && ($key != zen_session_name()) && ($key != 'error') && (!in_array($key, $exclude_array)) && ($key != 'x') && ($key != 'y') ) {
...
function zen_get_products_length_status($products_id){
    global $db;
    $result = $db->Execute("select products_length_status from products where products_id = '$products_id'");
    if($result->fields['products_length_status'] == 1){
        return true;
    }else{
        return false;
    }
}
function zen_get_all_get_params_sort($exclude_array = '', $search_engine_safe = true,$sort='') {
    global $cPath_array;
    $narrow_by_hidding = false;
    $target_categories = array(897,898,1120);
    $value = array();
    ...
    if (is_array($_GET) && (sizeof($_GET) > 0)) {
        ...
        while (list($key, $value) = each($_GET)) {
            ...
            if ('narrow' == $key && sizeof($_GET['narrow'])) {

                array_unique($value);
                array_pop($value);   //针对排序，去掉narrow最后一个元素

                sort($value,SORT_NUMERIC);
                foreach ($value as $i => $v){
                    //part 2
                    if ($narrow_by_hidding) {
                        $sql = "SELECT products_narrow_by_options_id FROM products_narrow_by_options_values_to_options WHERE products_narrow_by_options_values_id = " .(int)$v;
                        $result = zen_get_data_from_db($sql, array('products_narrow_by_options_id'));
                        if (!in_array($result[0][0],array(337,340,339,345,341,342,343))) {
                            $get_url.= '&'.$key.'[]='.$v.'&';
                        }
...
function zen_round($value, $precision=2) {
...
//print_r($params);
    $uprid = $prid;
    if ( (is_array($params) && !empty($params)) && (!strstr($prid, ':')) ) {
        ksort($params);
        reset($params);
        while (list($option, $value) = each($params)) {
            if (strstr($option, TEXT_PREFIX)) {
                ...
                if (is_array($value)) {
                    ...
                    if(strpos($option,'upload_prefix_') !== false){
                        $uprid = $uprid . '{' . $option . '}' . trim($value['products_options_value_text']);
                    }else{
...
function zen_db_input($string) {
    return zen_db_prepare_input($string);
}
 ...
 * 批量插入数据
 * @param $table
 * @param array $fields 需要插入的字段名
 * @param array $data 需要插入的数据（二维数组如：array(array(val1,val2),array(val3,val4)...);）
 * @return bool|queryFactoryResult|null
 */
function zen_db_batch_inserts($table, array $fields, array $data)
{
    global $db;
 ...
 *
 * @param $table string
 * @param $datas  array[array]
 * @return bool
 */
function zen_db_inserts($table, $datas) {
    global $db;
        ...
        foreach ($keys as $key){
            ...
            switch ((string)$v) {
                ...
                case 'null':
                    ...
                    break;
                default:
                    $value .= '\'' . zen_db_input($v) . '\', ';
                    break;
            }
        }
        $insert[] = substr($value, 0, -2) . ')';
    }
        ...
        while ($file=readdir($dh)) {
            if($file!="." && $file!="..") {
                ...
                if(!is_dir($fullpath)) {
                    ...
                    delCache($fullpath,$all);
                }
            }
        }
    }
}
function delOneCache($pid){
    if(file_exists(DIR_FS_CATALOG.'cache/products/'.md5("products_id_".$pid).'.html')){

        delCache(DIR_FS_CATALOG.'cache/products/'.md5("products_id_".$pid).'.html');
    }
}
...
function getUserBrowse()
{
    ...
    if(ereg('Opera', $Agent)) {
        ...
        $browseinfo = EMAIL_BODY_COMMON_UNKNOWN;
    }
    return $browseinfo;
}
...
function getUserOS(){
    ...
    if(stristr($agent,'iPad')) {
        ...
        preg_match('/^.*?\(.*;(.*?)Build.*?\).*?$/',$agent,$match);
        $login_device = trim($match[1]);
        if($login_device){  // 获取手机的具体型号
            $fb_fs = $login_device;
        }else{
        ...
        $fb_fs = EMAIL_BODY_COMMON_UNKNOWN;
    }
    return $fb_fs;
}
...
function getCountryByGeoIP(){
    global $db;
    $geoData = geoip_open('GeoIP.dat', GEOIP_STANDARD);
    $countryCode = geoip_country_code_by_addr($geoData, getCustomersIP());
    $sql = 'select countries_name from  countries where countries_iso_code_2="'.$countryCode.'" limit 1 ';
    $countries = $db->getAll($sql);
    $countries_name = $countries[0]['countries_name'];
    return $countries_name;
}

// 2018.??? fairy 添加数据库时间
function insert_db_time($arr,$is_add=false,$customer_id){
    $customer_id = $customer_id?$customer_id:$_SESSION ['customer_id'];
    $time = date('Y-m-d H:i:s');
    if($is_add){
        $arr['created_by'] = $customer_id;
        $arr['creation_date'] = $time;
    }
    $arr['last_update_date'] = $time;
    $arr['last_updated_by'] = $customer_id;

    return $arr;
}

// 2018.12.21 fairy 只替换一次
...
// get countries
require(DIR_WS_FUNCTIONS . 'functions_getCountry.php');
//get length
require(DIR_WS_FUNCTIONS . 'functions_length.php');
////
