...
*/
  function zen_get_countries($countries_id = '', $with_iso_codes = false) {
    ...
    if (zen_not_null($countries_id)) {
      ...
      $countries = "select countries_id, countries_name, tel_prefix
                    from " . TABLE_COUNTRIES . "
                    order by countries_name";
...
//    $new_range = ' and date_format(p.products_date_available, \'%Y%m%d\') >' . $zc_new_date;
  ...
  function fs_get_rewrite_url($type,$id){
  	switch ($type){
  		case 'category':
  			return HTTP_SERVER.'/'.strtolower(preg_replace('/(\/|[[:space:]]{1,}|\"|\\\')/i', '-', ltrim(zen_get_categories_name($id)))).fs_get_categories_path($id);
  			break;
  		case 'product':
  			return HTTP_SERVER.'/'.strtolower(preg_replace('/(\/|[[:space:]]{1,}|\"|\\\')/i', '-', ltrim(zen_get_products_name($id)))).'-p'.$id . '.html';
  			break;
