...
class order_quotes extends base
{

    var $info, $totals, $products, $customer, $delivery, $content_type, $email_low_stock, $products_ordered_attributes, $sendTo, $billTo, $products_ordered, $products_ordered_email, $attachArray, $is_reissue, $local_warehouse, $global_warehouse, $warehouse, $currencies_value, $delay_cart_products, $local_cart_products, $global_cart_products, $shipping_warehouse, $is_buck, $is_buck_in_products, $heavy_products, $local_shipping_cost, $delay_shipping_cost, $global_shipping_cost, $heavy_products_tag, $is_local_buck, $cart_products, $is_remote_post_code, $local_cabinet, $special_heavy, $spec_heavy_arr, $local_heavy_products, $is_heavy_free, $vax, $insertData, $insurance, $is_de_remote, $is_lithium_battery_order, $shoppingCartProducts, $orderVat, $is_send_trustpilot, $is_local_oversize, $is_delay_oversize, $is_local_overweight, $is_delay_overweight, $is_ireland_zones,$is_ireland_billings_zones, $cn_limit_products;
     ...
     */
    function order_quotes($order_id = '', $cart_products = [], $sendTo = 0, $billTo = 0)
    {
        $this->delivery_id = "";
        $this->info = array();
        $this->totals = array();
    ...
    function query($order_id)
    {
        ...
        $order_query = "select customers_id, customers_name,customers_lastname, customers_company,
                         ...
                         billing_street_address, billing_suburb, billing_city, billing_postcode,billing_tax_numberbilling_company_type,
    ...
    function cart()
    {
        ...
        if ($_SESSION['customer_id']) {
            ...
            $customer_address_query = "select c.first_name as customers_firstname, c.last_name as customers_lastname,
                                    
                                        c.telephone as customers_telephone,
        ...
        if (!empty($_SESSION['sendto_quotes'])) {
            $shipping_address_query = "select ab.entry_firstname, ab.entry_lastname, ab.entry_company,
                ...
                TABLE_COUNTRIES . " as c where ab.entry_country_id = c.countries_id and ab.address_book_id = " . (int)$_SESSION['sendto_quotes'];
            $this->delivery_id = (int)$_SESSION['sendto_quotes'];
        ...
        switch (STORE_PRODUCT_TAX_BASIS) {
            case 'Shipping':
                $tax_address_query = "select ab.entry_country_id, ab.entry_zone_id
                              ...
                              and ab.address_book_id = '" . (int)($this->content_type == 'virtual' ? $_SESSION['billto_quotes'] : $_SESSION['sendto_quotes']) . "'";
            ...
            case 'Billing':
                $tax_address_query = "select ab.entry_country_id, ab.entry_zone_id
                              ...
                              and ab.address_book_id = '" . (int)$_SESSION['billto_quotes'] . "'";
        ...
        $this->local_info = array('order_status' => DEFAULT_ORDERS_STATUS_ID,
            ...
            'shipping_method' => (is_array($_SESSION['shipping_quotes']) ? $_SESSION['shipping_quotes']['id'] : $_SESSION['shipping_quotes']),

            'shipping_module_code' => (isset($_SESSION['shipping_quotes']['id']) ? $_SESSION['shipping_quotes']['id'] : $_SESSION['shipping_quotes']),
            ...
            'shipping_cost' => $_SESSION['shipping_quotes']['cost'],
            ...
            'comments' => (isset($_SESSION['comments_quotes']) ? $_SESSION['comments_quotes'] : ''),
...
//    $this->info['shipping_cost'] = zen_round(get_products_all_currency_final_price($this->info['shipping_cost'] * $currencies_value),2)/$currencies_value;
    ...
    {
        global $db;
        if (!empty($_SESSION['billto_quotes'])) {
            ...
            $billing_address_query = "select ab.entry_firstname, ab.entry_lastname, ab.entry_company,
                ...
                TABLE_COUNTRIES . " as c where ab.entry_country_id = c.countries_id and ab.address_book_id = " . (int)$_SESSION['billto_quotes'];
    ...
    function create($zf_ot_modules, $zf_mode = 2, $zf_ot_modules_tax = [])
    {
        ...
        if ($this->info['total'] == 0) {
            ...
            if (DEFAULT_ZERO_BALANCE_ORDERS_STATUS_ID == 0) {
                ...
                if ($_SESSION['payment_quotes'] != 'freecharger') {
        ...
        if ($_SESSION['shipping_quotes'] == 'free_free') {

            $this->info['shipping_module_code'] = $_SESSION['shipping_quotes'];
        ...
        $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
            ...
            'customers_remarks' => !empty($_SESSION['customer_remarks_quotes']) ? $_SESSION['customer_remarks_quotes'] : "",
        ...
        if ($_SESSION['shipping_quotes']['title'] == 'customzones') {
    ...
    function create_separate_order_info($tax_address, $products)
    {
        ...
        if (german_warehouse("country_code", $country_code) || other_eu_warehouse($country_code, "country_code")) {
            ...
            $this->is_vax = true;
            $this->is_vax_de = true;
            if (time() > 1609451999) { //柏林2020-12-31 23：59：59时间戳
                $vax_num = 19;
            } else {
                $vax_num = 16;
            }
    ...
    public function resetOrderTotalInfo()
    {
        ...
        $country_code = $this->delivery['country']["iso_code_2"];
        $billing_country_code = strtoupper($this->billing['country']['iso_code_2']);
        $this->orderVat = $_SESSION['orderVat_quotes'];
        ...
        $this->local_shipping_cost = $local_shipping_cost = $this->handleShippingCost($_SESSION['shipping_local_quotes']['cost']);
        $this->delay_shipping_cost = $delay_shipping_cost = $this->handleShippingCost($_SESSION['shipping_delay_quotes']['cost']);
        $this->global_shipping_cost = $global_shipping_cost = $this->handleShippingCost($_SESSION['shipping_global_quotes']['cost']);
        //保险费
        if ($_SESSION['shipping_delay_quotes']['id'] == 'forwarderzones_forwarderzones') {
            ...
            if (($is_confirm == 0)) {
                ...
                if ($update_status) {
                    if (in_array($warehouse, array(3, 20, 2, 37, 40, 67, 71))) {
                        ...
                        if ($warehouse == 3) {
                            $remain_num = fs_products_instock_qty_of_products_id($products[$i]['id'], "US", false, $pcs = $products[$i]['quantity'], false, $composite_data);
                        } elseif ($warehouse == 20) {
                            $remain_num = fs_products_instock_qty_of_products_id($products[$i]['id'], "DE", false, $pcs = $products[$i]['quantity'], false, $composite_data);
                        } elseif ($warehouse == 37) {
                            $remain_num = fs_products_instock_qty_of_products_id($products[$i]['id'], "AU", false, $pcs = $products[$i]['quantity'], false, $composite_data);
                        } elseif ($warehouse == 40) {
                            $remain_num = fs_products_instock_qty_of_products_id($products[$i]['id'], "US-ES", false, $pcs = $products[$i]['quantity'], false, $composite_data);
                        } elseif ($warehouse == 67) {
                            $remain_num = fs_products_instock_qty_of_products_id($products[$i]['id'], "RU", false, $pcs = $products[$i]['quantity'], false, $composite_data);
                        } elseif ($warehouse == 71) {
                            $remain_num = fs_products_instock_qty_of_products_id($products[$i]['id'], "SG", false, $pcs = $products[$i]['quantity'], false, $composite_data);
                        } else {
                            $remain_num = fs_products_instock_qty_of_products_id($products[$i]['id'], "CN", true, $pcs = $products[$i]['quantity'], false, $composite_data);
                        }
                ...
                case 71:
                    switch ($order_data) {
                        case "local":
                            ...
                            $session_shipping_local = $_SESSION['shipping_local_quotes'];
                        ...
                        case "delay":
                            ...
                            $session_shipping_delay = $_SESSION['shipping_delay_quotes'];
                            $is_reissue = 14;
                        ...
                        case "global":
                            ...
                            $session_shipping_global = $_SESSION['shipping_global_quotes'];
                            $is_reissue = 15;
...
//                                $current_warehouse = $this->global_warehouse;
                    ...
                    case "delay-global":
                        ...
                        switch ($this->local_warehouse) {
                            case 40:
                                ...
                                if ($_SESSION['shipping_delay_quotes']['id'] == "fedexiezones_fedexiezones") {
    ...
    function _create_order_new($zf_ot_modules, $main_id, $set_info, $is_reissue, $warehouse = "", $shippings, $account_info = array(), $zf_ot_modules_tax = [])
    {
        ...
        if ($set_info['total'] == 0) {
            ...
            if (DEFAULT_ZERO_BALANCE_ORDERS_STATUS_ID == 0) {
                ...
                if ($_SESSION['payment_quotes'] != 'freecharger' && !in_array($is_reissue, [22, 23])) {
        ...
        if ($_SESSION['shipping_quotes'] == 'free_free') {
            ...
            $set_info['shipping_module_code'] = $_SESSION['shipping_quotes'];
        ...
        if ($this->delivery['country']['id'] == 223) {
            if ($is_reissue == 12) {
                ...
                $products['local'][] = [
                    'qty' => 1,
                    'type' => 'shipping',
                    'name' => $_SESSION['shipping_local_quotes']['title'],
                    'id' => $_SESSION['shipping_local_quotes']['id'],
                ...
                $products['delay'][] = [
                    'qty' => 1,
                    'type' => 'shipping',
                    'name' => $_SESSION['shipping_delay_quotes']['title'],
                    'id' => $_SESSION['shipping_delay_quotes']['id'],
    ...
    {
        global $db;
        if (!empty($_SESSION['echeck_info_quotes']) && $this->info['payment_module_code'] == "echeck" && $orders_id) {
            ...
            $check_info = array(
                "orders_id" => $orders_id,
                "account_name" => $_SESSION['echeck_info_quotes']["account_name"] ? $_SESSION['echeck_info_quotes']["account_name"] : "",
                "account_number" => $_SESSION['echeck_info_quotes']["account_number"] ? $_SESSION['echeck_info_quotes']["account_number"] : "",
                "account_type" => $_SESSION['echeck_info_quotes']["account_type"] ? $_SESSION['echeck_info_quotes']["account_type"] : "",
                "routing_number" => $_SESSION['echeck_info_quotes']["account_route"] ? $_SESSION['echeck_info_quotes']["account_route"] : "",
    ...
    {
        if (!empty($_SESSION['related_email_quotes'])) {
            $email_arr = $_SESSION['related_email_quotes'];
...
//                }
    ...
    function send_order_email($zf_insert_id, $zf_mode)
    {
        ...
        /* 第三部分 */
        if ($purchase == 'alfa') {
            $alfa_info_array = ['alfa_contact_person', 'alfa_phone', 'alfa_email', 'alfa_organization', 'alfa_inn', 'alfa_kpp', 'alfa_okpo', 'alfa_bic', 'alfa_legal_address', 'alfa_postal_address', 'alfa_correspondent_accout', 'alfa_bank_name', 'alfa_settlement_account', 'alfa_holder_name'];
...
//        $this->heavy_products_tag = array_unique($this->heavy_products_tag);
    ...
    {
        $alfa_data = array();
        if (!empty($_SESSION['alfa_info_quotes']) && $this->info['payment_module_code'] == "alfa" && $orders_id) {
            $service = new \App\Services\Payments\RuPaymentServices();
            $service->setInformationAddress();
            if (!empty($_SESSION['alfa_info_quotes'])) {
                $data = $service->firstData($_SESSION['alfa_info_quotes']);
            } else {
            ...
            if (empty($alfa_data)) {
                $alfa_data = $_SESSION['alfa_info_quotes'];
...
//        $FSEmail = get_mail_site_and_country();
    ...
    public function createSgTicketNumber($orderId)
    {
        ...
        $res = false;
        try {
            $temp = $_SESSION['delivery_array_quotes'];
            if (!empty($temp)) {
                $data = [
                    'orders_id' => $orderId,
                ...
                $server->create($data);
                if (isset($_SESSION['delivery_array_quotes'])) {
                    unset($_SESSION['delivery_array_quotes']);
                }
                $res = true;
            }
        } catch (\Exception $e) {
    ...
    function handleFreeShippingPrice($local_shipping_method = '', $delay_shipping_method = '')
    {
        ...
        if ($local_shipping_method == '' && $delay_shipping_method == '') {
            $local_shipping_method = $_SESSION['shipping_local_quotes']['id'];
            $delay_shipping_method = $_SESSION['shipping_delay_quotes']['id'];
