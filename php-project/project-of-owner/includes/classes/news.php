...
class newsDisplay {
	...
	function articleListing($article_array) {
		...
		foreach ( $article_array as $thisArticle ) {
			$this->news_page .= '<li><a href="' . $thisArticle ['link'] . '">' . $thisArticle ['text'] . '</a></li>';
		}
	...
	function articleFooter($heading, $footer_array, $footer_date_url = false, $footer_date_text = false, $split_after = true) {
		if (is_array ( $footer_array )) {
			...
			foreach ( $footer_array as $thisFooter ) {
				$this->news_page .= '  <li><a href="' . $thisFooter ['link'] . '">' . $thisFooter ['text'] . '</a></li>' . "\n";
			}
			...
			if ($footer_date_url && $footer_date_text) {
				$this->clearSplit ();
				$this->news_page .= '<p class="articleText"><a href="' . $footer_date_url . '">' . $footer_date_text . '</a></p>' . "\n";
			}
	...
	function articleBackLink($link, $link_text) {
		$this->news_page .= '<p class="backLink"><a href="' . $link . '">' . $link_text . '</a></p>' . "\n";
	...
	function archiveLink($archive_url, $archive_url_text) {
		...
		$this->news_page .= '<p class="articleText"><a href="' . $archive_url . '">' . $archive_url_text . '</a></p>' . "\n";
	...
	function archiveListBegin() {
		...
		$this->news_page .= '<div class="clearSplit"><a name="archive"></a><hr /></div>' . "\n";
	...
	function clearSplit() {
		$this->news_page .= '<div class="clearSplit"><hr /></div>' . "\n";
		
		return true;
	}
	
	function splitSolid() {
		$this->news_page .= '<div class="splitSolidGray"><hr /></div>' . "\n";
	 ...
	 * @param unknown_type $article_array
	 */
	function index_news_list($article_array){
		$i=1;$n = sizeof($article_array);
		foreach ( $article_array as $thisArticle ) {
			$this->news_page .= '<a href="' . $thisArticle ['link'] . '">' . $thisArticle ['text'] . '</a>';
			if (++$i < $n) $this->news_page .= '<br/>'."\n";
		}
		return true;
	}
