...
class order_total extends base {
      ...
      while (list(, $value) = each($this->modules)) {
        ...
        for ($i=0, $n=sizeof($GLOBALS[$class]->output); $i<$n; $i++) {
          if (zen_not_null($GLOBALS[$class]->output[$i]['title']) && zen_not_null($GLOBALS[$class]->output[$i]['text'])) {
            $order_total_array[$class] = array('code' => $GLOBALS[$class]->code,
  ...
  function process_new($set_info) {
    ...
    if (is_array($this->modules)) {
      ...
      while (list(, $value) = each($this->modules)) {
        $class = substr($value, 0, strrpos($value, '.'));
        if (!isset($GLOBALS[$class])) continue;
        $GLOBALS[$class]->process_new($set_info);
        for ($i=0, $n=sizeof($GLOBALS[$class]->output_new); $i<$n; $i++) {
          if (zen_not_null($GLOBALS[$class]->output_new[$i]['title']) && zen_not_null($GLOBALS[$class]->output_new[$i]['text'])) {
            $order_total_array[$class] = array('code' => $GLOBALS[$class]->code,
                'title' => $GLOBALS[$class]->output_new[$i]['title'],
                'text' => $GLOBALS[$class]->output_new[$i]['text'],
                'value' => $GLOBALS[$class]->output_new[$i]['value'],
                'sort_order' => $GLOBALS[$class]->sort_order);
          }
        }
      }
  ...
  function shipping_total($oid){
  global $db;
  $shipping_total_array = array();
  $totals = $db->Execute("select title, text,value, class
