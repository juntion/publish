...
// 				$this->watermark['file'] = ($ihConf['large']['watermark']) ? $ihConf['dir']['docroot'] . $ihConf['dir']['images'] . 'large/watermark' . $ihConf['large']['suffix'] . '.png' : '';
	...
	function get_local_s() {
		//if ($this->local) return $this->local;
	
		// check if image handler is available and if we should resize at all
		if ($this->resizing_allowed()) {
			
			$this->local = $this->get_resized_image_s($this->width, $this->height);
		} else {
			$this->local = $this->src;
		}
		return $this->local;
	}
	function get_local_seo() {
		//if ($this->local) return $this->local;

		// check if image handler is available and if we should resize at all
		if ($this->resizing_allowed()) {

			$this->local = $this->get_resized_image_seo($this->width, $this->height);
		} else {
			$this->local = $this->src;
		}
		return $this->local;
	}
	function get_local_a() {
		if ($this->local) return $this->local;
		// check if image handler is available and if we should resize at all
			$this->local = $this->get_resized_image_a($this->width, $this->height);
		return $this->local;
	}
  ...
  function resizing_allowed() {
    ...
    return $allowed;
       	...
       	function get_resized_image_a($width, $height, $override_sizetype = '', $filetype = '') {
		global $ihConf;
      ...
      default:
		...
		list($newwidth, $newheight, $resize) = $this->calculate_size_a($width, $height);
		// set canvas dimensions
		$newwidth = 550;
		$newheight = 550;
		if (($newwidth > 0) && ($newheight > 0)) {
	...
	function get_resized_image_seo($width, $height, $override_sizetype = '', $filetype = ''){
		global $ihConf;
		$sizetype = ($override_sizetype == '') ? $this->sizetype : $override_sizetype;
		switch ($sizetype) {
			case 'large':
		...
		if ($resize || ($this->watermark['file'] != '') || ($this->zoom['file'] != '') || ($file_extension != $this->extension)){
			...
			if ( (($mtime > @filemtime($this->filename)) && ($mtime > @filemtime($this->watermark['file'])) && ($mtime > @filemtime($this->zoom['file'])) ) ||
				$this->resize_imageIM($file_extension, $local, $background, $quality) ||
				$this->resize_imageGD_s($file_extension, $local, $background, $quality) ) {
				return str_replace($ihConf['dir']['docroot'], '', $local);
			}
			//still here? resizing failed
		}
	...
	function get_resized_image_s($width, $height, $override_sizetype = '', $filetype = ''){
		global $ihConf;
		$sizetype = ($override_sizetype == '') ? $this->sizetype : $override_sizetype;
		switch ($sizetype) {
			case 'large':
	...
	function calculate_size($pref_width, $pref_height = '') {
		...
		return array($newwidth, $newheight, $resize);
        	...
        	function calculate_size_a($pref_width, $pref_height = '') {
		list($width, $height) = @getimagesize($this->filename);
		// default: nothing happens (preferred dimension = actual dimension)
		$newwidth = $width;
		$newheight = $height;
  ...
  function resize_imageGD_s($file_ext, $dest_name, $bg, $quality = 85) {
  	global $ihConf;
  	global $messageStack;
