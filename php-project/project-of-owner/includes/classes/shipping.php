  ...
  function shipping($module = '',$shipping_list= []) {
    ...
    if (defined('MODULE_SHIPPING_INSTALLED') && zen_not_null(MODULE_SHIPPING_INSTALLED)) {
      ...
      for ($i=0, $n=sizeof($include_modules); $i<$n; $i++) {
        ...
        if ($this->enabled && file_exists(DIR_WS_MODULES . 'shipping/' . $include_modules[$i]['file']))
        {
...
//        $shipping_num_boxes = ceil($shipping_weight/SHIPPING_MAX_WEIGHT);
  ...
  function quote($method = '', $module = '', $calc_boxes_weight_tare = true, $order_tag = false) {
    $quotes_array = array();
    ...
    if (is_array($this->modules)) {
      ...
      for ($i=0; $i<$size; $i++) {
        $quotes = $GLOBALS[$include_quotes[$i]]->quote($method,$order_tag);
        
        if (is_array($quotes)) $quotes_array[$quotes['id']] = $quotes;
      }
    }
   ...
   // by henly 2013-7-3
    function quotes($method = '',$total_weight, $include_quotes = '', $country, $post_code, $price, $state, $is_buck, $length_array,$zone_type,$is_shipping_free=false,$products_info = [])
    {
        $quotes_array = array();
        $total_weight = $total_weight;
        if (is_array($include_quotes)) {
            $size = sizeof($include_quotes);
            for ($i = 0; $i < $size; $i++) {
                if($GLOBALS[$include_quotes[$i]]){
                    $quotes = $GLOBALS[$include_quotes[$i]]->quotes($method, $total_weight, $country, $post_code, $price, $state, $is_buck, $length_array, $zone_type,$is_shipping_free,$products_info);
                    if (is_array($quotes)) $quotes_array[$quotes['id']] = $quotes;
                }
            }
        }
