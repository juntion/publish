...
class messageStack extends base {
  ...
  function add($class, $message, $type = 'error') {
    ...
    if (strlen($message) > 0) {
     if ($type == 'error') {
        $theAlert = array('params' => 'class=""  ', 'class' => $class, 'text' => $message,'type' => $type);
      } elseif ($type == 'warning') {
        ...
        $theAlert = array('params' => 'class="messageStackSuccess larger tishi_01"  ', 'class' => $class, 'text' => $message,'type' => $type);
      } elseif ($type == 'caution') {
...
/* here is the block*/
//      $theAlert['text'] .= '<span id="clos" onclick="toggle()"></span>';
  ...
  function output($class) {
    ...
    for ($i=0, $n=sizeof($this->messages); $i<$n; $i++) {
      if ($this->messages[$i]['class'] == $class) {
        ...
        if ('success' == $this->messages[$i]['type']) {
        	$type='success';;
        }
      }
    }
...
//    $output = array_values(array_unique($output));
    ...
    if (sizeof($output)){
    	if ($type =='error') {
    		...
    		require($template->get_template_dir('tpl_message_stack_success_default.php',DIR_WS_TEMPLATE, $current_page_base,'templates'). '/tpl_message_stack_success_default.php');
    	}
    }
