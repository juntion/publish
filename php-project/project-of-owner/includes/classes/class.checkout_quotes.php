...
class Checkout
{
    ...
    public static function getAddressForm($type = 1, $shipping_type = 1, $is_show_email = false, $order = '')
    {
        ...
        $state = self::getState(strtoupper($_SESSION['countries_iso_code_quotes']));
        $tpl_prefix = self::getCountriesTelPhonePrefix(strtoupper($_SESSION['countries_iso_code_quotes']));
    ...
    public static function getValidate($add_valid = array(), $country_id = 0)
    {
        ...
        $shipping_validate = array(
            ...
            "messages" => array(
                ...
                "entry_suburb" => [
                    "maxlength" => FS_SURBSTREET_MAXLENGTH_ERROR,
    ...
    public static function validate($address, $vat_check = true)
    {
        ...
        //Vat验证
        $EU_country = array(21, 222, 73, 81, 105, 150, 124, 57, 103, 195, 84, 171, 14, 203, 72, 132, 55, 170, 97, 56, 189, 190, 67, 117, 123, 175, 33, 53, 141, 244, 222);
...
EOF;
    ...
    public static function handlerShippingAddress($address_book_id = "", $is_valide = false, $is_guest = false, $error_param = [], $order, $shipping_data)
    {
        ...
        $price_data = array(
            ...
            "vat_title" => get_checkout_vat_title($order->delivery['country']['iso_code_2'], 2, $is_bill_company),
            //产品价格信息
            ...
            'after_shipping_text' => $shipping_price_tax == FIBER_CHECK_FREE ? FIBER_CHECK_FREE : $symbol . $shipping_price_tax . $symbol_right,
        ...
        $countries_iso_code = $_SESSION['countries_iso_code_quotes'];
    ...
    public static function setShippingCost($shipping_data)
    {
        $shipping_arr = ["local", "delay", "global", 'gift'];
        foreach ($shipping_arr as $e) {
            if ($shipping_data[$e]) {
                $shipping_cost = $shipping_data[$e];
                $_SESSION['shipping_' . $e . '_quotes'] = array(

                    'id' => $shipping_cost[0]['methods'] . '_' . $shipping_cost[0]['methods'],

                    'title' => $shipping_cost[0]['title'],

                    'cost' => $shipping_cost[0]['s_price'],

                    'origin_price' => $shipping_cost[0]['origin_price'],
                );
            } else {
                unset($_SESSION['shipping_' . $e . '_quotes']);
            }
        }
    }
        ...
        if ($shipping_address['entry_country_id'] == 223) {
            ...
            if (!empty($order->local_products)) {
                ...
                $local_products[] = [
                    ...
                    'paypal_price' => $currencies->total_format_new($order->handleShippingCost($_SESSION['shipping_local_quotes']['cost']),
            ...
            if (!empty($order->delay_products)) {
                ...
                $delay_products[] = [
                    ...
                    'paypal_price' => $currencies->total_format_new($order->handleShippingCost($_SESSION['shipping_delay_quotes']['cost']),
                        true, $order->info['currency'], $order->info['currency_value'])
                ];
            }
            $avaTax->setAddress([
            ...
            if ($action != 'calculateVax') {
                $_SESSION['useUpsDefaultAddress_quotes'] = 0;
            }
                ...
                if ($action != 'calculateVax') {
                    ...
                    if ($avaTaxCheck['status'] == false) {
                        if ($avaTaxCheck['type'] == 'validate') {
                            ...
                            $_SESSION['useUpsDefaultAddress_quotes'] = 1;
            ...
            //计算税收
            if ($calculateVax) {
                if ($_SESSION['useUpsDefaultAddress_quotes']) {
                    $avaTax->changeShipToAddress('line1', $avaTax->defaultLine);
                }
                ...
                if ($data['status']) {
                    ...
                    $_SESSION['avaTaxRecord_quotes'] = @json_encode($data['data']);
                    $_SESSION['orderVat_quotes'] = $orderVat;
                } else {
                    unset($_SESSION['avaTaxRecord_quotes']);
    ...
    public static function productInfo($order = "")
    {
        ...
        switch ($warehouse) {
            case 40:
                if (!empty($local_products)) {
                    ...
                    $delivery_info = array(
                        'shipping_local' => $_SESSION['shipping_local_quotes'],
        ...
        if (!empty($delay_products)) {
            ...
            if ($_SESSION['shipping_delay_quotes']['id']) {
                $shipping_method_arr = explode('_', $_SESSION['shipping_delay_quotes']['id']);
    ...
    public static function create_products_for_checkout($products, $order, $tag)
    {
        ...
        foreach ($products as $k => $v) {
            ...
            $productArray[$k]['price_total'] = $currencies->display_price_rate(zen_round(($v['paypal_price'] *
                $currencies->currencies[$_SESSION['currency']]['value']), 2), $v['tax'], $v['qty']);
            ...
            $productArray[$k]['price_total_tax'] = $currencies->display_price_rate(zen_round(($aftertax_products_price *
                $currencies->currencies[$_SESSION['currency']]['value']), 2), $v['tax'], $v['qty']);
