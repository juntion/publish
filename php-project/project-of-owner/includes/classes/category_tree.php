...
class category_tree extends base {
  ...
  function get_fiberstore_category_tree(){
  	global $db;
  	$categories = array();
  	$categories_query = "select c.categories_id, cd.categories_name, c.parent_id, c.categories_image
                               from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd
  	...
  	if ($get_categories->RecordCount()){
  		while (!$get_categories->EOF){
  			$categories[$get_categories->fields['categories_id']] = array(
  			'name' => $get_categories->fields['categories_name'],
            'parent' => 0,
  			'has_subs' => false);
  			if ($db->Execute("select count(c.categories_id)
                               from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd
                               where c.parent_id = ".(int)$get_categories->fields['categories_id']." 
                               and c.categories_id = cd.categories_id
                               and cd.language_id=" . (int)$_SESSION['languages_id'] . "
                               and c.categories_status= 1
                               order by sort_order, cd.categories_name")->RecordCount())
  			$categories['has_subs'] = true;
  			$this->get_category_tree_for_parent($categories, $get_categories->fields['categories_id']);
  ...
  function get_category_tree_for_parent(& $category, $parent_id){
  	  global $db;
      $categories_query = "select c.categories_id, cd.categories_name, c.parent_id, c.categories_image
                               from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd
  	...
  	if ($get_categories->RecordCount()){
  		while (!$get_categories->EOF){
  			$category[$get_categories->fields['categories_id']] = array(
  			'name' => $get_categories->fields['categories_name'],
            'parent' => $parent_id,
  			'has_subs' => false);
  			if ($db->Execute("select count(c.categories_id)
                               ...
                               and c.categories_status= 1
                               order by sort_order, cd.categories_name")->RecordCount())
  			$category['has_subs'] = true;
  			$this->get_category_tree_for_parent($category, $get_categories->fields['categories_id']);
  			$get_categories->MoveNext();
  			/*this category is the last one*/
  			if ($get_categories->EOF) $category[$get_categories->fields['categories_id']]['is_last'] = true;
  		}
  	}
  }
}
