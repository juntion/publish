...
//$public_files = getDir(DIR_WS_LANGUAGES.$_SESSION['language'].'/public');
//while(list ($key, $value) = each($public_files)){
//	require_once($value);        //加载public共用所有语言包
//}

// 首页优化。文件进行整合，把public文件夹下面的文件整理成一个文件进行调用。
// fairy 2019.6.19 add
$public_files = DIR_WS_LANGUAGES.$_SESSION['language'].'/public/common.php';
if (file_exists($public_files)) {
    require_once($public_files);
}
