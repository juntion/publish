...
if (!$contaminated) {
  foreach($paramsToCheck as $key) {
    if (isset($_GET[$key]) && !is_array($_GET[$key])) {
      ...
      if (isset($_GET[$key]) && strlen($_GET[$key]) > 100) {
        $contaminated = true;
        break;
      }
    }
...
/**
 * ery 2019.11.04
 * 数据库初始化配置 需要放在includes/configure.php文件加载之后，不然数据库相关配置无法调取
 */
...
/**
 * 如果 未定义网站根目录 或者是class目录则出现 zencard 安装
 */
 ...
 * narrow by  设置
 * ob 缓存 指定开启页面
 * keyword sort Get参数 过滤处理
 * id products_id 参数进行过滤处理
 * 图片处理配置设置
 */
...
/**
 * 加载initsystem 列出的文件
 * shoppingCart 和save Cart 会被存入session
 * notfiy messageStack 会被初始化
 */
...
if(file_exists(DIR_FS_CATALOG.'includes/classes/customClass.php')){
  require_once(DIR_FS_CATALOG.'includes/classes/customClass.php');
}
