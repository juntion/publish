...
<?php 
 $article_categories_id  = zen_get_solution_article_id_of_category_id($_GET['s_id']);
    ...
    ?>
         ...
         <?php for ($i=0, $n = sizeof($solution_array); $i < $n; $i++){
            $sub = $db->Execute("select doc_parent_id from solution_categories where doc_categories_id=".(int)$article_categories_id);
         ?>
                  
		                      <dl class="solution_nav_01" id="nav"> 
		                      <dt>
		                      <?php if($solution_array[$i]['doc_categories_id'] ==  $sub->fields['doc_parent_id'] && $_GET['s_id'] != '' ){?>
		...
		where doc_categories_id = '".intval($pid)."' and a.doc_article_status = 1 and ad.language_id = ".(int)$_SESSION['languages_id']." 
